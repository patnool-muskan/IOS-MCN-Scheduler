...
static int do_one_dlsch(unsigned char *input_ptr, PHY_VARS_gNB *gNB, NR_gNB_DLSCH_t *dlsch, int slot)
{
  ...
  #ifdef UNI_RAN
  if (rel15->rbSize == 0 || rel15->NrOfSymbols == 0 || rel15->nrOfLayers == 0) {
    LOG_W(PHY, "Invalid PDSCH parameters for UE (rbSize=%d, NrOfSymbols=%d, nrOfLayers=%d), skipping\n",
          rel15->rbSize, rel15->NrOfSymbols, rel15->nrOfLayers);
    return 0;
  }

  //Case n_dmrs == 0 -> Handling
  if (n_dmrs == 0) {
    LOG_W(PHY, "Skipping DLSCH: n_dmrs=0 (BWPStart=%d rbStart=%d rbSize=%d numDmrsCdmGrpsNoData=%d)\n",
          rel15->BWPStart, rel15->rbStart, rel15->rbSize, rel15->numDmrsCdmGrpsNoData);
    return 0;
  }
  #else
