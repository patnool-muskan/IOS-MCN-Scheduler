  ...
  for (int ant = 0; ant < frame_parms->nb_antennas_rx; ant++) {
    for (int p_index = 0; p_index < N_ap; p_index++) {
      ...
      for (int srs_symb = 0; srs_symb < (1 << srs_pdu->num_symbols); srs_symb++) {
        ...
        c16_t *srs_estimated_channel16 = (c16_t *)&srs_est[subcarrier + srs_symbol_offset + mem_offset];
        ...
        for (int k = 0; k < M_sc_b_SRS; k++) {
          ...
          srs_ls_estimated_channel[subcarrier + srs_symbol_offset] = ls_estimated;
        ...
        const uint16_t sc_offset = subcarrier + mem_offset;
          ...
          if (srs_pdu->comb_size == 0) {
            if (k == 0) { // First subcarrier case
              ...
              // filt8_start is {12288,8192,4096,0,0,0,0,0}
              srs_estimated_channel16 = (c16_t *)&srs_est[subcarrier + srs_symbol_offset];
              ...
              srs_estimated_channel16 = (c16_t *)&srs_est[sc_offset + srs_symbol_offset];
            }
          } else {
            if (k == 0) { // First subcarrier case
