...
#if 0
if (pdusession_modify_resp_p->nb_of_pdusessions > 0) {
    asn1cSequenceAdd(out->protocolIEs.list,
    ...
    for (int i = 0; i < pdusession_modify_resp_p->nb_of_pdusessions; i++) {
        asn1cSequenceAdd(
            ie->value.choice.PDUSessionResourceModifyListModRes.list,
            NGAP_PDUSessionResourceModifyItemModRes_t, item);
        item->pDUSessionID =
            pdusession_modify_resp_p->pdusessions[i].pdusession_id;

        /* Allocate transfer container */
        NGAP_PDUSessionResourceModifyResponseTransfer_t *transfer =
            calloc(1, sizeof(*transfer));
        if (!transfer) {
            NGAP_ERROR("Failed to alloc transfer\n");
            continue;
        }

        /* Add QFI(s) to qosFlowAddOrModifyResponseList */
        asn1cCalloc(transfer->qosFlowAddOrModifyResponseList, tmp);
        for (int qos_flow_index = 0;
             qos_flow_index <
             pdusession_modify_resp_p->pdusessions[i].nb_of_qos_flow;
             qos_flow_index++) {
            if(qos_flow_index  == 0)
              continue;
            asn1cSequenceAdd(tmp->list,
                             NGAP_QosFlowAddOrModifyResponseItem_t, qos);
            qos->qosFlowIdentifier =
                pdusession_modify_resp_p->pdusessions[i]
                    .qos[qos_flow_index]
                    .qfi;
        }

        /* Encode transfer into OCTET STRING */
        asn_encode_to_new_buffer_result_t res = asn_encode_to_new_buffer(
            NULL, ATS_ALIGNED_CANONICAL_PER,
            &asn_DEF_NGAP_PDUSessionResourceModifyResponseTransfer, transfer);

        if (res.result.encoded < 0) {
            NGAP_ERROR("Failed to encode PDUSessionResourceModifyResponseTransfer\n");
            ASN_STRUCT_FREE(asn_DEF_NGAP_PDUSessionResourceModifyResponseTransfer,
                            transfer);
            continue;
        }

        /* Copy encoded buffer into NGAP IE */
        item->pDUSessionResourceModifyResponseTransfer.buf = res.buffer;
        item->pDUSessionResourceModifyResponseTransfer.size =
            res.result.encoded;

        /* Free only the struct, keep encoded buffer */
        ASN_STRUCT_FREE(asn_DEF_NGAP_PDUSessionResourceModifyResponseTransfer,
                        transfer);
