...
#define XNAP_SN_STATUS_TRANSFER(mSGpTR) (mSGpTR)->ittiMsg.xnap_xn_sn_status_transfer /* Xn code for SN status transfer procedures */
...
typedef struct pdcp_count_sn_18_s {
 int pdcp_sn18;
 int hfn_pdcp_sn18;
} pdcp_count_sn_18_t;

typedef struct pdcp_count_sn_12_s {
 int pdcp_sn12;
 int hfn_pdcp_sn12;
} pdcp_count_sn_12_t;

typedef struct pdcp_sn_18bits_s {
 pdcp_count_sn_18_t count_value;
} pdcp_sn_18bits_t;

typedef struct pdcp_sn_12bits_s {
 pdcp_count_sn_12_t count_value;
} pdcp_sn_12bits_t;

typedef struct pdcp_status_transfer_s {
 pdcp_sn_12bits_t pdcp_sn_12bit;
 pdcp_sn_18bits_t pdcp_sn_18bit;
} pdcp_status_transfer_t;

typedef struct drbs_subject_to_transfer_item_s {
  uint8_t drb_id;
  pdcp_status_transfer_t pdcp_status_ul;
  pdcp_status_transfer_t pdcp_status_dl;
} drbs_subject_to_transfer_item_t;

typedef struct drbs_subject_to_transfer_list_s {
  uint8_t num_drb; // number of drbs to be transfer is 1 for now, need to update later
  drbs_subject_to_transfer_item_t drb[0]; //XNAP_MAX_NUM_DRBS is 1 for now; need to be modify later
} drbs_subject_to_transfer_list_t;

typedef struct xnap_xn_sn_status_transfer_s {
  uint32_t s_ng_node_ue_xnap_id;
  uint32_t t_ng_node_ue_xnap_id;
  drbs_subject_to_transfer_list_t drb_list;
} xnap_xn_sn_status_transfer_t;
