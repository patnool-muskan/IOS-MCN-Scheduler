...
static void f1ap_write_flows_mapped(const f1ap_flows_mapped_to_drb_t *flows_mapped, F1AP_Flows_Mapped_To_DRB_List_t *asn1_flows_mapped, int n)
{
  for (int k = 0; k < n; k++) {
    ...
    if (type == NON_DYNAMIC_5QI) {
      ...
      /* OPTIONAL delayCritical */
      if (0) {
        asn1cCallocOne(QosParams->choice.dynamic_5QI->delayCritical, 1L);
      }

      /* OPTIONAL averagingWindow */
      if (0) {
        asn1cCallocOne(QosParams->choice.dynamic_5QI->averagingWindow, 1L);
      }

      /* OPTIONAL maxDataBurstVolume */
      if (0) {
        asn1cCallocOne(QosParams->choice.dynamic_5QI->maxDataBurstVolume, 1L);
      }
    }
    ...
    if (flow_qos_params_in->gbr_qos_flow_info) {
      ...
      if (0) {
        asn1cCallocOne(flow_item->qoSFlowLevelQoSParameters.gBR_QoS_Flow_Information->maxPacketLossRateDownlink, 1L);
      }

      /* OPTIONAL maxPacketLossRateUplink */
      if (0) {
        asn1cCallocOne(flow_item->qoSFlowLevelQoSParameters.gBR_QoS_Flow_Information->maxPacketLossRateUplink, 1L);
      }
    }

    /* OPTIONAL reflective_QoS_Attribute */
    if (0) {
      asn1cCallocOne(flow_item->qoSFlowLevelQoSParameters.reflective_QoS_Attribute, 1L);
    }
  }
}
...
int CU_send_UE_CONTEXT_SETUP_REQUEST(sctp_assoc_t assoc_id, f1ap_ue_context_setup_t *f1ap_ue_context_setup_req)
{
  ...
  /* Create */
  /* 0. Message Type */
  LOG_I(F1AP,"F1AP UEContextSetupRequest called\n");
  ...
  if (f1ap_ue_context_setup_req->cu_to_du_rrc_information!=NULL) {
    ...
    if (f1ap_ue_context_setup_req->cu_to_du_rrc_information->handoverPreparationInfo_length > 0) {
      LOG_I(F1AP,"F1AP UEContextSetupRequest Inside handover prepration\n");
...
{
  LOG_I(F1AP,"F1AP CU handle UE context setup response \n");
  MessageDef                       *msg_p;
...
{
  LOG_I(F1AP,"F1AP CU handle UE context mod response \n");
  MessageDef                       *msg_p;
