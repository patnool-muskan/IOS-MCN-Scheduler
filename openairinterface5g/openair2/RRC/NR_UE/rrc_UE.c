...
static void nr_rrc_ue_process_rrcReconfiguration(NR_UE_RRC_INST_t *rrc, int gNB_index, NR_RRCReconfiguration_t *reconfiguration)
{
  ...
  switch (reconfiguration->criticalExtensions.present) {
	...
	//Commenting below to avoid nrUE crash
...
void *rrc_nrue(void *notUsed)
{
  ...
  case NAS_REG_REQ_IND:
    ...
    nas_reg_req_ind_t *ind = &NAS_REG_REQ_IND(msg_p);
    rrc->nasRegReqMsg.length = ind->nasMsg.length;
    rrc->nasRegReqMsg.nas_data = calloc(ind->nasMsg.length, sizeof(*ind->nasMsg.nas_data));
    memcpy(rrc->nasRegReqMsg.nas_data, ind->nasMsg.nas_data, rrc->nasRegReqMsg.length);
    break;
...
static void nr_rrc_initiate_rrcReestablishment(NR_UE_RRC_INST_t *rrc, NR_ReestablishmentCause_t cause)
{
  ...
  nr_rlc_discard_all_pdus(rrc->ue_id, true);
  nr_rlc_discard_all_pdus(rrc->ue_id, false);
