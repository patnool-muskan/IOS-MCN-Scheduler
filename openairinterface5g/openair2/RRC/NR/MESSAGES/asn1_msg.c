...
int do_NR_RRCRelease(uint8_t *buffer, size_t buffer_size, uint8_t Transaction_id)
{
  ...
  /* allocate the non-critical extension container (RRCRelease-v1540-IEs) */
  rrcConnectionRelease->criticalExtensions.choice.rrcRelease->nonCriticalExtension =
      CALLOC(1, sizeof(struct NR_RRCRelease_v1540_IEs));
  /* allocate the waitTime integer and set it (value must be 1..16) */
  long *wait_time_ptr = CALLOC(1, sizeof(long));
  *wait_time_ptr = 16; 
  rrcConnectionRelease->criticalExtensions.choice.rrcRelease->nonCriticalExtension->waitTime = wait_time_ptr;
...
int16_t prepare_DL_DCCH_for_HO_Command(const gNB_RRC_UE_t *UE,
                        ...
                        size_t buffer_size)
{
    LOG_I(NR_RRC, "HO LOG : prepare DL_DCCH for HO Command \n");
...
#ifdef UNI_RAN
int do_NR_Paging(uint8_t Mod_id, uint8_t *buffer, uint64_t tmsi)
{
  ...
  BIT_STRING_t *ng_5G_S_TMSI = &paging_record_p->ue_Identity.choice.ng_5G_S_TMSI;
  memset(ng_5G_S_TMSI, 0, sizeof(*ng_5G_S_TMSI));

  uint8_t tmsi_bytes[6];
  for (int i = 0; i < 6; i++)
    tmsi_bytes[i] = (tmsi >> (8*(5 - i))) & 0xFF;

  ng_5G_S_TMSI->buf = CALLOC(1, 6);
  memcpy(ng_5G_S_TMSI->buf, tmsi_bytes, 6);
  ng_5G_S_TMSI->size = 6;
  ng_5G_S_TMSI->bits_unused = 0;
...
int16_t do_NR_HandoverCommand(uint8_t *ho_buf, int16_t ho_size, uint8_t *rrc_buffer, int16_t rrc_size)
{
  ...
  AssertFatal(enc_rval.encoded > 0, "ASN1 message encoding failed (%s, %lu)!\n", enc_rval.failed_type->name, enc_rval.encoded);
  //ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_NR_HandoverCommand, &ho_command);
...
int do_NR_HandoverPreparationInformation(const uint8_t *uecap_buf, int uecap_buf_size, uint8_t *buf, int buf_size, uint8_t *rrc_reconf_buf, uint32_t rrc_reconf_len)
{
  ...
  if ((rrc_reconf_buf != NULL) && (rrc_reconf_len > 0)) {
    asn1cCalloc(hpi_ie->sourceConfig, sourceConfig);
    OCTET_STRING_fromBuf(&sourceConfig->rrcReconfiguration,
                       (const char *)rrc_reconf_buf, rrc_reconf_len);
  }
