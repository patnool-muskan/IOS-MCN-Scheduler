...
bool trigger_bearer_setup(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, int n, pdusession_t *sessions, uint64_t ueAggMaxBitRateDownlink)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < pdu->numDRB2Setup; j++) {
...
#else
      //For multiple QOS flows in one  DRB per PDU session
      pdu->numDRB2Setup = 1;
      int drb_id = get_next_available_drb_id(UE);

      drb_t *rrc_drb = generateDRB_multiQos(UE,
                                            drb_id,
                                            pduSession->param.pdusession_id,
                                            rrc->configuration.enable_sdap,
                                            rrc->security.do_drb_integrity,
      ...
      DRB_nGRAN_to_setup_t *drb = pdu->DRBnGRanList;
      drb->id = rrc_drb->drb_id;
#endif
      /* SDAP */
...
#ifdef UNI_RAN
      /* Map ALL QFIs from session->qos[] into the DRB’s SDAP config */
      for (int k=0; k < session->nb_qos; k++) {
          pdusession_level_qos_parameter_t *qos_session = session->qos + k;
          drb->sdap_config.mappedQoS_FlowsToAdd[k] = qos_session->qfi;
          LOG_I(NR_RRC, "Mapped QFI %d (5QI %d) into DRB %d for PDU session %d\n", qos_session->qfi, qos_session->fiveQI, rrc_drb->drb_id, session->pdusession_id);
      }
#endif
      /* PDCP */
      ...
      memset(drb->cellGroupList, MCG, drb->numCellGroups * sizeof(*drb->cellGroupList));
  ...
  UE->e1_assoc_id_xn = assoc_id; // Xn HO code
  LOG_E(NR_RRC,"ASSOC ID IN TRIGGER BEARER %d\n", assoc_id); // Xn HO code
...
//------------------------------------------------------------------------------
{
  ...
  for (int i = 0; i < req->nb_pdusessions_tomodify; i++) {
...
#else
    pdusession_t *sessMod = req->pdusession_modify_params + i;
#endif
    ...
    if (sess == UE->pduSession + UE->nb_of_pdusessions) {
...
#ifdef UNI_RAN
      int sess_index = sess - UE->pduSession;
#endif
      all_failed = false;
...
#else
        UE->pduSession[sess_index].param.nas_pdu = sessMod->nas_pdu;
#endif
      }
...
#else
      decodePDUSessionResourceModify(&sess->param, UE->pduSession[sess_index].param.pdusessionTransfer);
#endif
      ...
      sessMod->nb_qos = 2;
      sessMod->qos[0].qfi = 1;
      sessMod->qos[0].fiveQI = 5;
      sessMod->qos[1].qfi = 2;
      sessMod->qos[1].fiveQI = 1;
      LOG_D(NR_RRC, "nb_qos(sessMod->param) %u\n", sessMod->nb_qos);
      for (int q = 0; q < sessMod->nb_qos; q++) {
        int new_qfi = sessMod->qos[q].qfi;
        int new_5qi = sessMod->qos[q].fiveQI;

        bool found = false;
        for (int k = 0; k < sess->param.nb_qos; k++) {
          if (sess->param.qos[k].qfi == new_qfi) {
            found = true;
            break;
          }
        }
        LOG_D(NR_RRC, "nb_qos(sess->param) %u\n", sess->param.nb_qos);
        if (!found && sess->param.nb_qos < QOSFLOW_MAX_VALUE) {
          int idx = sess->param.nb_qos++;
          sess->param.qos[idx].qfi   = new_qfi;
          sess->param.qos[idx].fiveQI = new_5qi;
          LOG_D(NR_RRC,
                "New addition : Added new QoS flow QFI=%d (5QI=%d) to PDU session %d\n",
                new_qfi, new_5qi, sess->param.pdusession_id);
...
int rrc_gNB_send_NGAP_PDUSESSION_MODIFY_RESP(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, uint8_t xid)
{
    ...
    for (int i = 0; i < UE->nb_of_pdusessions; i++) {
        if (xid != UE->pduSession[i].xid) {
            LOG_W(NR_RRC, "xid mismatch (pdu session index %d, status %d, xid %d/%d)\n",
        ...
        if (UE->pduSession[i].status == PDU_SESSION_STATUS_DONE) {
            ...
            if (!pduSession) {
                LOG_W(NR_RRC, "PDU SESSION modify of non-existing PDU session %d\n",
                ...
                resp->pdusessions_failed[pdu_sessions_failed].cause.type = NGAP_CAUSE_RADIO_NETWORK;
                resp->pdusessions_failed[pdu_sessions_failed].cause.value = NGAP_CAUSE_RADIO_NETWORK_UNKNOWN_PDU_SESSION_ID;
                pdu_sessions_failed++;
                continue;
            }
            ...
            for (int q = 0; q < UE->pduSession[i].param.nb_qos; q++) {
                LOG_I(RRC, "nb_qos = %u, pdu session modifcation response\n", UE->pduSession[i].param.nb_qos);
                pduSession->param.qos[q] = UE->pduSession[i].param.qos[q];
            }

            // Fill response PDU session info
            resp->pdusessions[pdu_sessions_done].pdusession_id = pduSession->param.pdusession_id;
            resp->pdusessions[pdu_sessions_done].nb_of_qos_flow = pduSession->param.nb_qos;

            for (int q = 0; q < pduSession->param.nb_qos; q++) {
                resp->pdusessions[pdu_sessions_done].qos[q].qfi = pduSession->param.qos[q].qfi;
                resp->pdusessions[pdu_sessions_done].qos[q].fiveQI = pduSession->param.qos[q].fiveQI;
                resp->pdusessions[pdu_sessions_done].qos[q].gbr_qos_flow_level_qos_params.gbr_dl = pduSession->param.qos[q].gbr_qos_flow_level_qos_params.gbr_dl;
                resp->pdusessions[pdu_sessions_done].qos[q].gbr_qos_flow_level_qos_params.gbr_ul = pduSession->param.qos[q].gbr_qos_flow_level_qos_params.gbr_ul;
                resp->pdusessions[pdu_sessions_done].qos[q].gbr_qos_flow_level_qos_params.mbr_dl = pduSession->param.qos[q].gbr_qos_flow_level_qos_params.mbr_dl;
                resp->pdusessions[pdu_sessions_done].qos[q].gbr_qos_flow_level_qos_params.mbr_ul = pduSession->param.qos[q].gbr_qos_flow_level_qos_params.mbr_ul;
            }

            LOG_I(NR_RRC,
                  "Modify Resp (msg index %d, pdu session index %d, status %d, xid %d): nb_of_pduSessions %d, pdusession_id %d\n",
                  pdu_sessions_done, i, pduSession->status, xid,
                  UE->nb_of_pdusessions, resp->pdusessions[pdu_sessions_done].pdusession_id);
            ...
            LOG_D(NR_RRC, "PDU SESSION is NEW or already ESTABLISHED (status %d)\n", UE->pduSession[i].status);
        }
    }
    ...
    if (pdu_sessions_done > 0 || pdu_sessions_failed > 0) {
        LOG_E(RRC, "NGAP_PDUSESSION_MODIFY_RESP: sending the message (total PDU sessions %d)\n", UE->nb_of_pdusessions);
        itti_send_msg_to_task(TASK_NGAP, rrc->module_id, msg_p);
    } else {
...
//----------------------------------------------------------------------------------------------
{
  ...
  UE->ho_context_xn->state = PSR_tExp; // May2 timers
  ...
  /* Security Context */
 /* nr_rrc_pdcp_config_security(UE, true); */ /* Apr 04 : srb 1 in wrongly updated with integrity 2 */
  ...
  if (ack->nb_pdusessions_switched > 0) {
    ...
    for (int i = 0; i < UE->nb_of_pdusessions; ++i) {
      // UE->pduSession[0].param.pdusession_id = ack->pdusessions_switched[0].pdusession_id; /* Xn code */
      ///// UE->pduSession[i] = pduSession[i];
      UE->pduSession[i].param = ack->pdusessions_switched[i];
      ///// LOG_E(NR_RRC,"pduSession[i].param.nb_qos is %d and UE->pduSession[i].param.nb_qos = %d\n", pduSession[i].param.nb_qos, UE->pduSession[i].param.nb_qos);
      LOG_E(NR_RRC,"UE->pduSession[i].param.pdusession_id is %d and UE->pduSession[i].param.nb_qos = %d\n", UE->pduSession[i].param.pdusession_id, UE->pduSession[i].param.nb_qos);
    }
  }
...
int rrc_gNB_process_Handover_Request(MessageDef* msg_p, instance_t instance)
{
  ...
  ue_context_p = rrc_gNB_create_ue_context(curr_assoc_id, UINT16_MAX, rrc, random_value, UINT32_MAX);
  ...
  nr_rrc_allocate_ho_ctx_target(rrc, UE, du);
  // Allocate context for the target DU (N2, Xn)
  ...
  // nr_initiate_handover(rrc, UE, NULL, du,UE->ue_handover_prep_info_buffer.buf ,UE->ue_handover_prep_info_buffer.len,1);

  //PREPARE PDCP
  set_UE_security_algos(rrc, UE, &handover_req->security_capabilities);
  //memset(UE->nh, 0, SECURITY_KEY_LENGTH);
  memcpy(UE->nh, handover_req->next_hop, SECURITY_KEY_LENGTH);
  LOG_I(NR_RRC, "HO LOG: pci %d ssb %d cip %ld int %d\n", cell_info->nr_pci,ssb_arfcn,UE->ciphering_algorithm,UE->integrity_algorithm);
  process_gNB_kgnb_star(rrc, UE, cell_info->nr_pci, ssb_arfcn);
...
#ifdef UNI_RAN
  nr_pdcp_add_srbs(true, UE->rrc_ue_id, SRBs,&null_security_parameters, true);
#else
  nr_pdcp_add_srbs(true, UE->rrc_ue_id, SRBs,&null_security_parameters);
#endif
  freeSRBlist(SRBs);
...
)
{
  ...
  MessageDef  *msg_p;
  msg_p = itti_alloc_new_message (TASK_RRC_GNB, rrc->module_id, NGAP_HANDOVER_REQUIRED);
  ...
  ho_required->sourceToTargetContainer->targetCellId.plmn_identity.mnc_digit_length = neighbourCellConfiguration->plmn.mnc_digit_length;
...
int rrc_gNB_process_NGAP_DL_RAN_STATUS_TRANSFER(MessageDef *msg_p, instance_t instance)
{
  ...
  for (int i = 0; i < cmd->ran_status.nb_drb; ++i) {
    ...
    if (i == 0)
      rrc->dl_srb_count_target = s->dl_count.pdcp_sn;
#endif
...
)
{
  ...
  MessageDef  *msg_p;
  msg_p = itti_alloc_new_message (TASK_RRC_GNB, rrc->module_id, NGAP_HANDOVER_NOTIFY);
...
)
{
  ...
  MessageDef  *msg_p;
  msg_p = itti_alloc_new_message (TASK_RRC_GNB, rrc->module_id, NGAP_HANDOVER_REQUEST_ACKNOWLEDGE);
...
void rrc_gNB_send_NGAP_PDUSESSION_RELEASE_RESPONSE(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, uint8_t xid)
{
  ...
  for (int i = 0; i < UE->nb_of_pdusessions; i++) {
    if (xid == UE->pduSession[i].xid) {
      int pdusession_id = UE->pduSession[i].param.pdusession_id;
      resp->pdusession_release[pdu_sessions_released].pdusession_id = pdusession_id;
      pdu_sessions_released++;
      // Clear
      drb_t *drb = find_drb_from_pduSession(UE, pdusession_id);
      if (drb)
...
{
  MessageDef *msg_p =
      itti_alloc_new_message(TASK_RRC_GNB,
                             rrc->module_id,
                             NGAP_PDUSESSION_RELEASE_RESPONSE);

  ngap_pdusession_release_resp_t *resp =
      &NGAP_PDUSESSION_RELEASE_RESPONSE(msg_p);
  ...
  /* =======================================================
   * Build NGAP Release Response list (READ-ONLY PHASE)
   * ======================================================= */
  for (int i = 0; i < UE->nb_of_pdusessions; i++) {
    if (UE->pduSession[i].xid == xid &&
        UE->pduSession[i].status == PDU_SESSION_STATUS_TORELEASE) {

      resp->pdusession_release[released_count].pdusession_id =
          UE->pduSession[i].param.pdusession_id;
  ...
  /* =======================================================
   * Cleanup UE state (NO RESPONSE MODIFICATION)
   * ======================================================= */
  for (int i = 0; i < UE->nb_of_pdusessions; i++) {
    if (UE->pduSession[i].xid == xid &&
        UE->pduSession[i].status == PDU_SESSION_STATUS_TORELEASE) {
      ...
      /* Release DRB belonging to this PDU session */
      drb_t *drb = find_drb_from_pduSession(UE, pdusession_id);
      if (drb)
        memset(drb, 0, sizeof(*drb));
      ...
      LOG_W(NR_RRC,
            "Released PDU session ID = %d for UE %x\n",
            pdusession_id, UE->rnti);
    }
  }

  /* =======================================================
   * Compact UE PDU session array (SAFE REMOVE)
   * ======================================================= */
  int new_count = 0;
  for (int i = 0; i < UE->nb_of_pdusessions; i++) {
    if (UE->pduSession[i].status != PDU_SESSION_STATUS_RELEASED) {
      UE->pduSession[new_count++] = UE->pduSession[i];
    }
  }
  UE->nb_of_pdusessions = new_count;

  /* =======================================================
   * Finalize and send NGAP message
   * ======================================================= */
  resp->nb_of_pdusessions_released = released_count;
  resp->nb_of_pdusessions_failed = 0;

  LOG_I(NR_RRC,
        "NGAP PDUSESSION RELEASE RESPONSE: rrc_ue_id %u released %d session(s)\n",
        resp->gNB_ue_ngap_id, released_count);
...
//------------------------------------------------------------------------------
{
  ...
  if (found) {
    if ((req.numPDUSessionsRem > 0) && NODE_IS_MONOLITHIC(RC.nrrrc[0]->node_type)) {
      if (ue_associated_to_cuup(rrc, UE)) {
...
int rrc_gNB_process_PAGING_IND(MessageDef *msg_p, instance_t instance)
{
  ...
  /* first two bytes: AMF-Set-ID (10b) | AMF-Pointer (6b), arranged as network-order bytes */
  s_tmsi_bytes[0] = (uint8_t)((amf_set_id >> 2) & 0xFF);                         // MSB of 16-bit field
  s_tmsi_bytes[1] = (uint8_t)(((amf_set_id & 0x3) << 6) | (amf_pointer & 0x3F)); // LSB of 16-bit field
  /* append 4 bytes of 5G-TMSI in big-endian */
  ...
  /* Debug prints: raw components and packed bytes */
  LOG_D(NR_RRC, "NGAP fields: amf_set_id=%u, amf_pointer=%u, 5g-tmsi=0x%08X\n",
      amf_set_id, amf_pointer, fiveg_tmsi);

  LOG_D(NR_RRC, "Packed S-TMSI bytes: %02X %02X %02X %02X %02X %02X\n",
      s_tmsi_bytes[0], s_tmsi_bytes[1], s_tmsi_bytes[2],
      s_tmsi_bytes[3], s_tmsi_bytes[4], s_tmsi_bytes[5]);

  uint64_t s_tmsi_uint64 = ((uint64_t)amf_set_id << 38) | ((uint64_t)amf_pointer << 32) | (uint64_t)fiveg_tmsi;
       ...
       for (uint8_t j = 0; j < RC.nrrrc[instance]->configuration.num_plmn; j++) {
           ...
           if (RC.nrrrc[instance]->configuration.mcc[j] == NGAP_PAGING_IND(msg_p).plmn_identity[tai_size].mcc
               ...
               for (uint8_t CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
                   /* Generate PCCH paging message directly at RRC */
                   rrc_gNB_generate_pcch_msg(assoc_id,
                                             s_tmsi_uint64,
                                             NGAP_PAGING_IND(msg_p).paging_drx,
                                             instance,
                                             CC_id);
               }
          }
...
set_timer(gNB_RRC_UE_t *UE, int duration){
        printf("+++ Setting timers +++\n");
        UE->ho_context_xn->state = PSR;
        UE->ho_context_xn->start_time = time(NULL);
        UE->ho_context_xn->duration = duration;
}


check_timers(){
  printf("+++ inside check timers \n");
  gNB_RRC_INST *rrc = RC.nrrrc[0];
  rrc_gNB_ue_context_t *ue_context_p = NULL;
  RB_FOREACH(ue_context_p, rrc_nr_ue_tree_s, &((gNB_RRC_INST *)rrc)->rrc_ue_head)
  {
    const gNB_RRC_UE_t *UE = &ue_context_p->ue_context;
    if(UE && UE->ho_context_xn && UE->ho_context_xn->state == PSR){
        printf("+++ time_lapsed = %lu +++\n", time(NULL)-UE->ho_context_xn->start_time);
        if (time(NULL) > UE->ho_context_xn->start_time + UE->ho_context_xn->duration){
            printf("++++ PSR_timer expired ++++\n");
            UE->ho_context_xn->state = PSR_tExp;
            rrc_gNB_send_XNAP_UE_CONTEXT_RELEASE(0, ue_context_p);
        }
    }
  }
}
