...
void nr_rrc_trigger_n2_ho(gNB_RRC_INST *rrc, uint32_t rrc_ue_id,const nr_neighbour_gnb_configuration_t *neighbour)
{
  ...
  if (target_du == NULL) {
    if (neighbour != NULL)
    {
      ho_ctx_type_t ctx_type = HO_CTX_SOURCE;                 
      nr_handover_context_t *ho_ctx = alloc_ho_ctx(ctx_type); 
      f1_ue_data_t ue_data = cu_get_f1_ue_data(ue->rrc_ue_id);
      if (source_du != NULL) {
        DevAssert(source_du->assoc_id == ue_data.du_assoc_id);
      ...
      ue->ho_context = ho_ctx;
      LOG_A(NR_RRC,
        "Handover Requred sent for UE %u/RNTI %04x \n",
        ue->rrc_ue_id,
        ue->rnti);
      rrc_gNB_Handover_Required(rrc, ue, neighbour);
    }
  } else {
...
void nr_rrc_allocate_ho_ctx_target(const gNB_RRC_INST *rrc,
                                  gNB_RRC_UE_t *ue,
                                  const nr_rrc_du_container_t *du)
{
  ho_ctx_type_t ctx_type = HO_CTX_TARGET;
  nr_handover_context_t *ho_ctx = alloc_ho_ctx(ctx_type);
  ho_ctx->target->du = du;
  // we will know target->{du_ue_id,new_rnti} once we have UE ctxt setup
  // response
  ho_ctx->target->ho_req_ack = nr_rrc_f1_ho_acknowledge;
  ho_ctx->target->ho_success = nr_rrc_f1_ho_complete;
  ue->ho_context = ho_ctx;
}
...
void nr_rrc_trigger_f1_ho(gNB_RRC_INST *rrc, gNB_RRC_UE_t *ue, nr_rrc_du_container_t *source_du, nr_rrc_du_container_t *target_du)
{
  ...
  int size = do_NR_HandoverPreparationInformation(ue->ue_cap_buffer.buf, ue->ue_cap_buffer.len, buf, sizeof buf,NULL,0);
...
bool is_valid_nr_cell_id(gNB_RRC_INST *rrc, uint64_t target_cell_id) {
  ...
  RB_FOREACH(neigh, rrc_neigh_cell_tree, &rrc->neighs) {
    if (neigh && neigh->nr_cellid == target_cell_id) {
      return true;
    }
  }
  return false;
...
void nr_HO_Xn_trigger_telnet(gNB_RRC_INST *rrc, uint32_t neighbour_pci, uint32_t rrc_ue_id){
  ...
  DevAssert(scell_du);
  LOG_E(NR_RRC, "UE %d: triggering Xn HO, serving cell (PCI=%d) to neighbour cell (PCI=%d)\n", rrc_ue_id, scell_pci, neighbour_pci);
 ...
 // const nr_neighbour_cell_t *neighbour = get_neighbour_cell_by_pci(cell, neighbour_pci);
  const nr_neighbour_gnb_configuration_t *neighbour =
            get_neighbour_cell_information(scell_du->nr_cellid, neighbour_pci);
  if (neighbour == NULL) {
    LOG_E(NR_RRC, "Xn HO trigger failed for UE %d: could not find neighbour cell with PCI=%d\n", rrc_ue_id, neighbour_pci);
    return;
  }

  bool xn_setup_established = is_valid_nr_cell_id(rrc, neighbour->nrcell_id);
  if(xn_setup_established){
      //nr_rrc_trigger_xn_ho(rrc, UE, scell_du->nr_pci, neighbour);
      nr_HO_Xn_trigger(UE->rrc_ue_id, neighbour);
  }else{
      LOG_W(NR_RRC,"Xn HO trigger failed for UE %d: No Xn-Setup Established for neighbour cell id %ld with pci %d \n", rrc_ue_id, neighbour->nrcell_id, neighbour_pci);
      return;
  }
}
