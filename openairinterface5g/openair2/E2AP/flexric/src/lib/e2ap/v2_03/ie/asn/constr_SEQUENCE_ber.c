 ...
 */
asn_dec_rval_t
SEQUENCE_decode_ber_e2ap_v2_03(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    case 1:
        ...
        for(edx = ((size_t)ctx->step >> 1); edx < td->elements_count;
            ...
            ASN_DEBUG("Current tag in %s SEQUENCE for element %" ASN_PRI_SIZE " "
                      ...
                      ber_tlv_tag_string_e2ap_v2_03(tlv_tag), (int)tag_len, (long)LEFT);
            switch(tag_len) {
            ...
            if(n == opt_edx_end) {
                ...
                if(!IN_EXTENSION_GROUP(specs,
                    ...
                    ASN_DEBUG("Unexpected tag %s (at %" ASN_PRI_SIZE ")",
                              ber_tlv_tag_string_e2ap_v2_03(tlv_tag), edx);
                    ASN_DEBUG("Expected tag %s (%s)%s",
                              ber_tlv_tag_string_e2ap_v2_03(elements[edx].tag),
                              elements[edx].name,
                    ...
                    ASN_DEBUG("Skipping unexpected %s (at %" ASN_PRI_SIZE ")",
                              ber_tlv_tag_string_e2ap_v2_03(tlv_tag), edx);
                    skip = ber_skip_length_e2ap_v2_03(opt_codec_ctx,
        ...
        microphase2:
            ...
            if(elements[edx].flags & ATF_OPEN_TYPE) {
                rval = OPEN_TYPE_ber_get_e2ap_v2_03(opt_codec_ctx, td, st, &elements[edx], ptr, LEFT);
            } else {
                rval = elements[edx].type->op->ber_decode_e2ap_v2_03r(opt_codec_ctx,
                                                           elements[edx].type,
                                                           memb_ptr2, ptr, LEFT,
    ...
    case 4:  /* only 00's expected */
        ...
        while(ctx->left) {
            ...
            tl = ber_fetch_tag_e2ap_v2_03(ptr, LEFT, &tlv_tag);
            switch(tl) {
            case 0: if(!SIZE_VIOLATION) RETURN(RC_WMORE);
                /* Fall through */
            case -1: RETURN(RC_FAIL);
            ...
            ll = ber_skip_length_e2ap_v2_03(opt_codec_ctx,
                                 BER_TLV_CONSTRUCTED(ptr),
 ...
 */
asn_enc_rval_t
SEQUENCE_encode_der_e2ap_v2_03(const asn_TYPE_descriptor_t *td, const void *sptr,
    ...
    for(edx = 0; edx < td->elements_count; edx++) {
        ...
        erval = elm->type->op->der_encode_e2ap_v2_03r(elm->type, *memb_ptr2,
     ...
     */
    ret = der_write_tags_e2ap_v2_03(td, computed_size, tag_mode, 1, tag, cb, app_key);
    ...
    for(edx = 0; edx < td->elements_count; edx++) {
        ...
        tmperval = elm->type->op->der_encode_e2ap_v2_03r(elm->type, *memb_ptr2,
