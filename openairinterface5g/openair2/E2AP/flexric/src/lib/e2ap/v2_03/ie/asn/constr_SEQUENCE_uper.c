...
asn_dec_rval_t
SEQUENCE_decode_uper_e2ap_v2_03(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    for(edx = 0;
        ...
        if(elm->flags & ATF_OPEN_TYPE) {
            rv = OPEN_TYPE_uper_get_e2ap_v2_03(opt_codec_ctx, td, st, elm, pd);
        } else {
    ...
    if(extpresent) {
        ...
        bmlength = uper_get_nslength_e2ap_v2_03(pd);
        ...
        for(;;) {
            ...
            case 0: continue;
            default:
                if(uper_open_type_skip_e2ap_v2_03(opt_codec_ctx, pd)) {
                    FREEMEM(epres);
                    ASN__DECODE_STARVED;
                }
...
SEQUENCE__handle_extensions(const asn_TYPE_descriptor_t *td, const void *sptr,
    ...
    for(edx = specs->first_extension; edx < td->elements_count; edx++) {
        ...
        if(po2 && present
           && uper_open_type_put_e2ap_v2_03(elm->type,
                                 elm->encoding_constraints.per_constraints,
                                 *memb_ptr2, po2))
            return -1;
    }
...
asn_enc_rval_t
SEQUENCE_encode_uper_e2ap_v2_03(const asn_TYPE_descriptor_t *td,
    ...
    if(uper_put_nslength_e2ap_v2_03(po, n_extensions))
        ASN__ENCODE_FAILED;
