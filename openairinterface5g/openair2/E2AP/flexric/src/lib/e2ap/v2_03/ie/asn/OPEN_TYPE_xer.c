...
asn_dec_rval_t
OPEN_TYPE_xer_get_e2ap_v2_03(const asn_codec_ctx_t *opt_codec_ctx,
     ...
     */
    for(;;) {
        ch_size = xer_next_token_e2ap_v2_03(&xer_context, ptr, size, &ch_type);
        if(ch_size < 0) {
            ASN__DECODE_FAILED;
        } else {
            switch(ch_type) {
     ...
     */
    switch(xer_check_tag_e2ap_v2_03(ptr, ch_size, elm->name)) {
    case XCT_OPENING:
        ADVANCE(ch_size);
        break;
    case XCT_BROKEN:
    ...
    rv = selected.type_descriptor->op->xer_decode_e2ap_v2_03r(
