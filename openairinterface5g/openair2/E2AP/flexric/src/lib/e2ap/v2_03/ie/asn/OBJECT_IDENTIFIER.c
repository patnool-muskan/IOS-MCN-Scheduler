...
asn_TYPE_operation_t asn_OP_OBJECT_IDENTIFIER_e2ap_v2_03 = {
    ...
    OCTET_STRING_compare_e2ap_v2_03,   /* Implemented in terms of a string comparison */
#if !defined(ASN_DISABLE_BER_SUPPORT)
    ber_decode_e2ap_v2_03_primitive_e2ap_v2_03,
    der_encode_e2ap_v2_03_primitive_e2ap_v2_03,
#else
    0,
    0,
#endif  /* !defined(ASN_DISABLE_BER_SUPPORT) */
#if !defined(ASN_DISABLE_XER_SUPPORT)
    OBJECT_IDENTIFIER_decode_xer_e2ap_v2_03,
    OBJECT_IDENTIFIER_encode_xer_e2ap_v2_03,
#else
    0,
    0,
    ...
    OBJECT_IDENTIFIER_random_fill_e2ap_v2_03,
#else
    0,
...
asn_TYPE_descriptor_t asn_DEF_OBJECT_IDENTIFIER_e2ap_v2_03 = {
        ...
        / sizeof(asn_DEF_OBJECT_IDENTIFIER_e2ap_v2_03_tags_e2ap_v2_03[0]),
    {
        ...
        OBJECT_IDENTIFIER_constraint_e2ap_v2_03
    },
    0, 0,  /* No members */
    0  /* No specifics */
};
...
int
OBJECT_IDENTIFIER_constraint_e2ap_v2_03(const asn_TYPE_descriptor_t *td, const void *sptr,
...
OBJECT_IDENTIFIER_get_first_arcs(const uint8_t *arcbuf, size_t arcbuf_len,
    ...
    ssize_t rd = OBJECT_IDENTIFIER_get_single_arc_e2ap_v2_03(arcbuf, arcbuf_len, &value);
    if(rd <= 0) return rd;
...
ssize_t
OBJECT_IDENTIFIER_get_single_arc_e2ap_v2_03(const uint8_t *arcbuf, size_t arcbuf_len,
...
ssize_t
OBJECT_IDENTIFIER__dump_body_e2ap_v2_03(const OBJECT_IDENTIFIER_t *st,
    ...
    for(off = rd; ; ) {
        asn_oid_arc_t arc;
        rd = OBJECT_IDENTIFIER_get_single_arc_e2ap_v2_03(st->buf + off, st->size - off,
                                              &arc);
        if(rd < 0) {
            return -1;
        } else if(rd == 0) {
...
ssize_t
OBJECT_IDENTIFIER_get_arcs_e2ap_v2_03(const OBJECT_IDENTIFIER_t *st, asn_oid_arc_t *arcs,
                           size_t arc_slots) {
    asn_oid_arc_t arc0 = 0;
    asn_oid_arc_t arc1 = 0;
    size_t num_arcs = 0;
 ...
 */
ssize_t
OBJECT_IDENTIFIER_set_single_arc_e2ap_v2_03(uint8_t *arcbuf, size_t arcbuf_len,
                                 asn_oid_arc_t value) {
    /*
...
int
OBJECT_IDENTIFIER_set_arcs_e2ap_v2_03(OBJECT_IDENTIFIER_t *st, const asn_oid_arc_t *arcs,
                           size_t arc_slots) {
    uint8_t *buf;
    uint8_t *bp;
    ssize_t wrote;
    ...
    wrote = OBJECT_IDENTIFIER_set_single_arc_e2ap_v2_03(bp, size, arc0 * 40 + arc1);
    if(wrote <= 0) {
        FREEMEM(buf);
        return -1;
    }
    ...
    for(i = 2; i < arc_slots; i++) {
		wrote = OBJECT_IDENTIFIER_set_single_arc_e2ap_v2_03(bp, size, arcs[i]);
        if(wrote <= 0) {
            FREEMEM(buf);
            return -1;
        }
...
ssize_t
OBJECT_IDENTIFIER_parse_arcs_e2ap_v2_03(const char *oid_text, ssize_t oid_txt_length,
...
#define _OID_CAPTURE_ARC(oid_text, oid_end)                       \
    do {                                                          \
        ...
        unsigned long value;                                      \
        switch(asn_strtoul_lim_e2ap_v2_03(oid_text, &endp, &value)) {        \
