 ...
 */
asn_dec_rval_t
SEQUENCE_decode_ber_e2ap_v3_01(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    case 0:
        ...
        rval = ber_check_tags_e2ap_v3_01(opt_codec_ctx, td, ctx, ptr, size,
    ...
    case 1:
        ...
        for(edx = ((size_t)ctx->step >> 1); edx < td->elements_count;
             ...
             */
            tag_len = ber_fetch_tag_e2ap_v3_01(ptr, LEFT, &tlv_tag);
            ASN_DEBUG("Current tag in %s SEQUENCE for element %" ASN_PRI_SIZE " "
                      "(%s) is %s encoded in %d bytes, of frame %ld",
                      td->name, edx, elements[edx].name,
                      ber_tlv_tag_string_e2ap_v3_01(tlv_tag), (int)tag_len, (long)LEFT);
            switch(tag_len) {
            ...
            if(n == opt_edx_end) {
                ...
                if(!IN_EXTENSION_GROUP(specs,
                    ...
                    ASN_DEBUG("Unexpected tag %s (at %" ASN_PRI_SIZE ")",
                              ber_tlv_tag_string_e2ap_v3_01(tlv_tag), edx);
                    ASN_DEBUG("Expected tag %s (%s)%s",
                              ber_tlv_tag_string_e2ap_v3_01(elements[edx].tag),
                              elements[edx].name,
                    ...
                    ASN_DEBUG("Skipping unexpected %s (at %" ASN_PRI_SIZE ")",
                              ber_tlv_tag_string_e2ap_v3_01(tlv_tag), edx);
                    skip = ber_skip_length_e2ap_v3_01(opt_codec_ctx,
        ...
        microphase2:
            ...
            if(elements[edx].flags & ATF_OPEN_TYPE) {
                rval = OPEN_TYPE_ber_get_e2ap_v3_01(opt_codec_ctx, td, st, &elements[edx], ptr, LEFT);
            } else {
                rval = elements[edx].type->op->ber_decode_e2ap_v3_01r(opt_codec_ctx,
                                                           elements[edx].type,
                                                           memb_ptr2, ptr, LEFT,
    ...
    case 4:  /* only 00's expected */
        ...
        while(ctx->left) {
            ...
            tl = ber_fetch_tag_e2ap_v3_01(ptr, LEFT, &tlv_tag);
            switch(tl) {
            case 0: if(!SIZE_VIOLATION) RETURN(RC_WMORE);
                /* Fall through */
            case -1: RETURN(RC_FAIL);
            ...
            || ctx->phase == 4) {
                ASN_DEBUG("Unexpected continuation "
                          ...
                          "%s (SEQUENCE): %s",
                          td->name,
                          ber_tlv_tag_string_e2ap_v3_01(tlv_tag));
                RETURN(RC_FAIL);
            }

            ll = ber_skip_length_e2ap_v3_01(opt_codec_ctx,
                                 BER_TLV_CONSTRUCTED(ptr),
 ...
 */
asn_enc_rval_t
SEQUENCE_encode_der_e2ap_v3_01(const asn_TYPE_descriptor_t *td, const void *sptr,
    ...
    for(edx = 0; edx < td->elements_count; edx++) {
        ...
        erval = elm->type->op->der_encode_e2ap_v3_01r(elm->type, *memb_ptr2,
     ...
     */
    ret = der_write_tags_e2ap_v3_01(td, computed_size, tag_mode, 1, tag, cb, app_key);
    ...
    for(edx = 0; edx < td->elements_count; edx++) {
        ...
        tmperval = elm->type->op->der_encode_e2ap_v3_01r(elm->type, *memb_ptr2,
