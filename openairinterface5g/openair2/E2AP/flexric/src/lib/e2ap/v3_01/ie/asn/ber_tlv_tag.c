...
ssize_t
ber_fetch_tag_e2ap_v3_01(const void *ptr, size_t size, ber_tlv_tag_t *tag_r) {
	ber_tlv_tag_t val;
...
ssize_t
ber_tlv_tag_fwrite_e2ap_v3_01(ber_tlv_tag_t tag, FILE *f) {
	char buf[sizeof("[APPLICATION ]") + 32];
	ssize_t ret;

	ret = ber_tlv_tag_snprint_e2ap_v3_01(tag, buf, sizeof(buf));
...
ssize_t
ber_tlv_tag_snprint_e2ap_v3_01(ber_tlv_tag_t tag, char *buf, size_t size) {
	const char *type = 0;
	int ret;
...
char *
ber_tlv_tag_string_e2ap_v3_01(ber_tlv_tag_t tag) {
	static char buf[sizeof("[APPLICATION ]") + 32];

	(void)ber_tlv_tag_snprint_e2ap_v3_01(tag, buf, sizeof(buf));
...
size_t
ber_tlv_tag_serialize_e2ap_v3_01(ber_tlv_tag_t tag, void *bufp, size_t size) {
