...
ssize_t
ber_fetch_length_e2ap_v3_01(int _is_constructed, const void *bufptr, size_t size,
		ber_tlv_len_t *len_r) {
...
ssize_t
ber_skip_length_e2ap_v3_01(const asn_codec_ctx_t *opt_codec_ctx,
	 ...
	 */
	ll = ber_fetch_length_e2ap_v3_01(_is_constructed, ptr, size, &vlen);
	if(ll <= 0) return ll;
	...
	for(skip = ll, ptr = ((const char *)ptr) + ll, size -= ll;;) {
		...
		/* Fetch the tag */
		tl = ber_fetch_tag_e2ap_v3_01(ptr, size, &tag);
		if(tl <= 0) return tl;
...
size_t
der_tlv_length_serialize_e2ap_v3_01(ber_tlv_len_t len, void *bufp, size_t size) {
