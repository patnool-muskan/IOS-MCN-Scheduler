 ...
 */
asn_dec_rval_t
xer_decode_e2ap_v3_01(const asn_codec_ctx_t *opt_codec_ctx,
	 ...
	 */
	return td->op->xer_decode_e2ap_v3_01r(opt_codec_ctx, td, struct_ptr, 0, buffer, size);
}
 ...
 */
ssize_t
xer_next_token_e2ap_v3_01(int *stateContext, const void *buffer, size_t size, pxer_chunk_type_e *ch_type) {
	struct xer__cb_arg arg;
	int new_stateContext = *stateContext;
	ssize_t ret;

	arg.callback_not_invoked = 1;
	ret = pxml_parse_e2ap_v3_01(&new_stateContext, buffer, size, xer__token_cb, &arg);
	if(ret < 0) return -1;
...
xer_check_tag_e2ap_v3_01_e
xer_check_tag_e2ap_v3_01(const void *buf_ptr, int size, const char *need_tag) {
	const char *buf = (const char *)buf_ptr;
	const char *end;
	xer_check_tag_e2ap_v3_01_e ct = XCT_OPENING;
	...
	if(!need_tag || !*need_tag)
		return (xer_check_tag_e2ap_v3_01_e)(XCT__UNK__MASK | ct);
 ...
 */
asn_dec_rval_t
xer_decode_e2ap_v3_01_general(const asn_codec_ctx_t *opt_codec_ctx,
	...
	for(;;) {
		...
		xer_check_tag_e2ap_v3_01_e tcv;		/* Tag check value */

		/*
		 * Get the next part of the XML stream.
		 */
		ch_size = xer_next_token_e2ap_v3_01(&ctx->context, buf_ptr, size,
			&ch_type);
		if(ch_size == -1) {
            RETURN(RC_FAIL);
        } else {
		...
		tcv = xer_check_tag_e2ap_v3_01(buf_ptr, ch_size, xml_tag);
		/*
		 * Phase 0:
...
size_t
xer_whitespace_span_e2ap_v3_01(const void *chunk_buf, size_t chunk_size) {
 ...
 */
int
xer_skip_unknown_e2ap_v3_01(xer_check_tag_e2ap_v3_01_e tcv, ber_tlv_len_t *depth) {
	assert(*depth > 0);
	switch(tcv) {
	case XCT_BOTH:
	case XCT_UNKNOWN_BO:
