...
asn_dec_rval_t
INTEGER_decode_aper_e2ap_v1_01(const asn_codec_ctx_t *opt_codec_ctx,
        ...
        if(ct->range_bits >= 0) {
            if (ct->range_bits > 16) {
                ...
                if((specs && specs->field_unsigned)
                        ? asn_uint642INTEGER_e2ap_v1_01(st, (unsigned long)value)
                        : asn_int642INTEGER_e2ap_v1_01(st, value))
                    ASN__DECODE_FAILED;
                ...
                value += ct->lower_bound;
                if((specs && specs->field_unsigned)
                        ? asn_ulong2INTEGER_e2ap_v1_01(st, value)
                        : asn_long2INTEGER_e2ap_v1_01(st, value))
                    ASN__DECODE_FAILED;
                ASN_DEBUG("Got value %ld + low %lld",
                          value, (long long int)ct->lower_bound);
            }
    ...
    do {
        ...
        /* Get the PER length */
        len = aper_get_length_e2ap_v1_01(pd, -1, -1, -1, &repeat);
    ...
    if(ct && ct->lower_bound) {
        ...
        if(asn_INTEGER2long_e2ap_v1_01(st, &value))
            ASN__DECODE_FAILED;
        if(asn_long2INTEGER_e2ap_v1_01(st, value + ct->lower_bound))
            ASN__DECODE_FAILED;
    }
...
asn_enc_rval_t
INTEGER_encode_aper_e2ap_v1_01(const asn_TYPE_descriptor_t *td,
    ...
    if(ct) {
        ...
        if(specs && specs->field_unsigned) {
            unsigned long uval;
            if(asn_INTEGER2ulong_e2ap_v1_01(st, &uval))
                ASN__ENCODE_FAILED;
            /* Check proper range */
            if(ct->flags & APC_SEMI_CONSTRAINED) {
            ...
            if(asn_INTEGER2long_e2ap_v1_01(st, &value)) ASN__ENCODE_FAILED;
            /* Check proper range */
            if(ct->flags & APC_SEMI_CONSTRAINED) {
    ...
    for(buf = st->buf, end = st->buf + st->size; buf < end;) {
        int need_eom = 0;
        ssize_t mayEncode = aper_put_length_e2ap_v1_01(po, -1, -1, end - buf, &need_eom);
        if(mayEncode < 0)
            ASN__ENCODE_FAILED;
