...
asn_dec_rval_t
NativeEnumerated_decode_aper_e2ap_v1_01(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    if(ct && ct->range_bits >= 0) {
        ...
        value = aper_get_nsnnwn_e2ap_v1_01(pd, 65537);
        if(value < 0) ASN__DECODE_STARVED;
...
asn_enc_rval_t
NativeEnumerated_encode_aper_e2ap_v1_01(const asn_TYPE_descriptor_t *td,
                             const asn_per_constraints_t *constraints,
    ...
    kf = bsearch(&key, specs->value2enum, specs->map_count,
                 sizeof(key), NativeEnumerated__compar_value2enum_e2ap_v1_01);
    if(!kf) {
    ...
    if(aper_put_nsnnwn_e2ap_v1_01(po,
