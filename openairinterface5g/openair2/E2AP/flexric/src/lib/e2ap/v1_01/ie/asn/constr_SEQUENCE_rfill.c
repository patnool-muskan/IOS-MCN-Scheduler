...
SEQUENCE_random_fill_e2ap_v1_01(const asn_TYPE_descriptor_t *td, void **sptr,
                   const asn_encoding_constraints_t *constr,
                   size_t max_length) {
    const asn_SEQUENCE_specifics_t *specs =
        (const asn_SEQUENCE_specifics_t *)td->specifics;
    asn_random_fill_e2ap_v1_01_result_t result_ok = {ARFILL_OK, 0};
    ...
    for(edx = 0; edx < td->elements_count; edx++) {
        ...
        asn_random_fill_e2ap_v1_01_result_t tmpres;

        if(elm->optional && asn_random_between_e2ap_v1_01(0, 4) == 2) {
