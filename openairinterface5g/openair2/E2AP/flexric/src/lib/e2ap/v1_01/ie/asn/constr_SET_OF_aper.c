...
asn_enc_rval_t
SET_OF_encode_aper_e2ap_v1_01(const asn_TYPE_descriptor_t *td,
    ...
    if(ct && ct->effective_bits >= 0) {
        ...
        if (aper_put_length_e2ap_v1_01(po, ct->lower_bound, ct->upper_bound, list->count - ct->lower_bound, 0) < 0) {
            ASN__ENCODE_FAILED;
        }
    }
     ...
     */
    encoded_els = SET_OF__encode_sorted_e2ap_v1_01(elm, list, SOES_CAPER);
    ...
    for(seq = 0; seq < list->count;) {
        ...
        if(ct && ct->effective_bits >= 0) {
            ...
            may_encode =
                aper_put_length_e2ap_v1_01(po, -1, -1, list->count - seq, &need_eom);
            if(may_encode < 0) ASN__ENCODE_FAILED;
        }

        while(may_encode--) {
            const struct _el_buffer *el = &encoded_els[seq++];
            if(asn_put_many_bits_e2ap_v1_01(po, el->buf,
    ...
    SET_OF__encode_sorted_e2ap_v1_01_free(encoded_els, list->count);

    ASN__ENCODED_OK(er);
}

asn_dec_rval_t
SET_OF_decode_aper_e2ap_v1_01(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    if(ct && ct->effective_bits >= 0) {
        /* X.691, #19.5: No length determinant */
        nelems = aper_get_nsnnwn_e2ap_v1_01(pd, ct->upper_bound - ct->lower_bound + 1);
    ...
    do {
        ...
        if(nelems < 0) {
            if (ct)
                nelems = aper_get_length_e2ap_v1_01(pd, ct->lower_bound, ct->upper_bound,
                                         ct->effective_bits, &repeat);
            else
                nelems = aper_get_length_e2ap_v1_01(pd, -1, -1, -1, &repeat);
        ...
        for(i = 0; i < nelems; i++) {
            ...
            rv = elm->type->op->aper_decode_e2ap_v1_01r(opt_codec_ctx, elm->type,
        ...
        nelems = -1;  /* Allow uper_get_length_e2ap_v1_01() */
    } while(repeat);
