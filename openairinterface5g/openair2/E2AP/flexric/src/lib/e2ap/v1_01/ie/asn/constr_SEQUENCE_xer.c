 ...
 */
asn_dec_rval_t
SEQUENCE_decode_xer_e2ap_v1_01(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    for(edx = ctx->step; ctx->phase <= 3;) {
        ...
        if(ctx->phase == 2) {
            ...
            if(elm->flags & ATF_OPEN_TYPE) {
                tmprval = OPEN_TYPE_xer_get_e2ap_v1_01(opt_codec_ctx, td, st, elm, ptr, size);
            } else {
         ...
         */
        ch_size = xer_next_token_e2ap_v1_01(&ctx->context, ptr, size,
            &ch_type);
        if(ch_size == -1) {
            RETURN(RC_FAIL);
        } else {
        ...
        tcv = xer_check_tag_e2ap_v1_01(ptr, ch_size, xml_tag);
        ...
        case XCT_UNKNOWN_BO:
            ...
            if(edx < td->elements_count) {
                ...
                for(n = edx; n < edx_end; n++) {
                    elm = &td->elements[n];
                    tcv = xer_check_tag_e2ap_v1_01(ptr, ch_size, elm->name);
                    switch(tcv) {
                    case XCT_BOTH:
                    case XCT_OPENING:
                        /*
...
asn_enc_rval_t
SEQUENCE_encode_xer_e2ap_v1_01(const asn_TYPE_descriptor_t *td, const void *sptr,
