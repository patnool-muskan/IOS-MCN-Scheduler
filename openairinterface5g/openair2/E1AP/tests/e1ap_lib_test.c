...
static void test_bearer_context_setup_request(void)
{
  ...
  e1ap_bearer_setup_req_t orig = {
      ...
      .pduSession[0].DRBnGRanList[0].sdap_config = dummy_sdap_config,
      .pduSession[0].DRBnGRanList[0].pdcp_config = dummy_pdcp_config,
      .pduSession[0].DRBnGRanList[0].id = 1,
      ...
      .pduSession[0].DRBnGRanList[0].qosFlows[0] = dummy_qos_flows,
  };
...
static void test_bearer_context_modification_request(void)
{
  ...
  *drb_to_setup.drb_inactivity_timer = 500;
  ...
  bool ret = decode_E1_bearer_context_mod_request(dec, &decoded);
  AssertFatal(ret, "decode_E1_bearer_context_mod_request(): could not decode message\n");
