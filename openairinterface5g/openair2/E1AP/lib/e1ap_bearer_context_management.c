...
static E1AP_PDU_Session_Resource_To_Modify_Item_t e1_encode_pdu_session_to_mod_item(const pdu_session_to_mod_t *in)
{
  ...
  for (const DRB_nGRAN_to_mod_t *j = in->DRBnGRanModList; j < in->DRBnGRanModList + in->numDRB2Modify; j++) {
    ...
    if (j->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn != 0)  {
      asn1cCalloc(drb2Mod->pdcp_SN_Status_Information, pdcp_SN_Status_Information);
      pdcp_SN_Status_Information->pdcpStatusTransfer_UL.countValue.pDCP_SN = j->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.pdcp_sn;
      pdcp_SN_Status_Information->pdcpStatusTransfer_UL.countValue.hFN = j->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.hfn;
      pdcp_SN_Status_Information->pdcpStatusTransfer_DL.pDCP_SN = j->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn;
      pdcp_SN_Status_Information->pdcpStatusTransfer_DL.hFN = j->pdcp_sn_status_info.pdcp_status_transfer_dl.hfn;
      // asn1cCallocOne(drb2Mod->pdcp_SN_Status_Information, E1AP_PDCP_SN_Status_Information);
      printf("fill_BEARER_CONTEXT_MODIFICATION_REQUEST ul sn %d ul hfn %d dl sn %d ul sn %d \n", j->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.pdcp_sn, j->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.hfn, j->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn, j->pdcp_sn_status_info.pdcp_status_transfer_dl.hfn);
      }
#endif
      /* E1AP PROCEDURES FOR SN STATUS TRANSFER */
...
#else    
    // PDCP SN Status Information (O)
    if (drb2Mod->pdcp_SN_Status_Information != NULL) {
         drb->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.pdcp_sn = drb2Mod->pdcp_SN_Status_Information->pdcpStatusTransfer_UL.countValue.pDCP_SN;
         drb->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.hfn = drb2Mod->pdcp_SN_Status_Information->pdcpStatusTransfer_UL.countValue.hFN;
         drb->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn = drb2Mod->pdcp_SN_Status_Information->pdcpStatusTransfer_DL.pDCP_SN;
         drb->pdcp_sn_status_info.pdcp_status_transfer_dl.hfn = drb2Mod->pdcp_SN_Status_Information->pdcpStatusTransfer_DL.hFN;
        printf("extract_BEARER_CONTEXT_MODIFICATION_REQUEST ul sn %d ul hfn %d dl sn %d dl hfn %d \n",drb2Mod->pdcp_SN_Status_Information->pdcpStatusTransfer_UL.countValue.pDCP_SN, drb2Mod->pdcp_SN_Status_Information->pdcpStatusTransfer_UL.countValue.hFN, drb2Mod->pdcp_SN_Status_Information->pdcpStatusTransfer_DL.pDCP_SN, drb2Mod->pdcp_SN_Status_Information->pdcpStatusTransfer_DL.hFN);
            }
#endif
    // DL UP TNL parameters (O)
  ...
  for (int i = 0; i < msg->numPDUSessionsMod; ++i) {
    ...
    if (pdu->numDRBModified > 0) {
      ...
      for (int j = 0; j < pdu->numDRBModified; ++j) {
        ...
        if (msg->sn_status_req) {
          drb_mod->pDCP_SN_Status_Information = calloc(1, sizeof (struct E1AP_PDCP_SN_Status_Information));
          drb_mod->pDCP_SN_Status_Information->pdcpStatusTransfer_UL.countValue.pDCP_SN = drb->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.pdcp_sn;
          drb_mod->pDCP_SN_Status_Information->pdcpStatusTransfer_UL.countValue.hFN = drb->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.hfn;
          drb_mod->pDCP_SN_Status_Information->pdcpStatusTransfer_DL.pDCP_SN = drb->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn;
          drb_mod->pDCP_SN_Status_Information->pdcpStatusTransfer_DL.hFN = drb->pdcp_sn_status_info.pdcp_status_transfer_dl.hfn;
         printf("if msg->sn_status_req in fill_BEARER_CONTEXT_MODIFICATION_RESPONSE ul sn %d ul hfn %d dl sn %d dl hfn %d \n",drb->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.pdcp_sn, drb->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.hfn, drb->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn, drb->pdcp_sn_status_info.pdcp_status_transfer_dl.hfn);
        }
        // Flow Setup List (O)
        ...
        for (int i = 0; i < list->list.count; i++) {
          ...
          if (pduIE->dRB_Modified_List_NG_RAN) {
            ...
            for (int j = 0; j < pduIE->dRB_Modified_List_NG_RAN->list.count; j++) {
              ...
              // DRB ID (M)
              drbMod->id = drbIE->dRB_ID;
              if(drbIE->pDCP_SN_Status_Information && drbIE->pDCP_SN_Status_Information->pdcpStatusTransfer_DL.pDCP_SN){
                  out->sn_status_req = 1;
                  drbMod->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.pdcp_sn = drbIE->pDCP_SN_Status_Information->pdcpStatusTransfer_UL.countValue.pDCP_SN;
                  drbMod->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.hfn = drbIE->pDCP_SN_Status_Information->pdcpStatusTransfer_UL.countValue.hFN;
                  drbMod->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn = drbIE->pDCP_SN_Status_Information->pdcpStatusTransfer_DL.pDCP_SN;
                  drbMod->pdcp_sn_status_info.pdcp_status_transfer_dl.hfn = drbIE->pDCP_SN_Status_Information->pdcpStatusTransfer_DL.hFN;
                 printf("extract_BEARER_CONTEXT_MODIFICATION_RESPONSE out->sn_status_req is %d dl sn %d \n", out->sn_status_req, drbIE->pDCP_SN_Status_Information->pdcpStatusTransfer_DL.pDCP_SN);
               }
              // UL UP Parameters (O)
