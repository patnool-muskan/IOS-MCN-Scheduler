...
typedef struct {
  atomic_ulong nr_pdcp_current_time;
  atomic_ulong nr_pdcp_current_time_last_frame;
  atomic_ulong nr_pdcp_current_time_last_subframe;
} nr_pdcp_time_t;

nr_pdcp_time_t nr_pdcp_last;

static uint64_t get_nr_pdcp_current_time(void)
{
  return nr_pdcp_last.nr_pdcp_current_time;
}

void nr_pdcp_tick(int frame, int subframe){
  nr_pdcp_time_t nr_pdcp_current = {
    .nr_pdcp_current_time = get_nr_pdcp_current_time()+1,
    .nr_pdcp_current_time_last_frame = frame,
    .nr_pdcp_current_time_last_subframe = subframe,
  };
  atomic_compare_exchange_strong(&nr_pdcp_current,&nr_pdcp_last,nr_pdcp_current);
  nr_pdcp_wakeup_timer_thread(nr_pdcp_current.nr_pdcp_current_time);
}
#elif
/* TODO: handle time a bit more properly */
static uint64_t      nr_pdcp_current_time;
static uint64_t      nr_pdcp_current_time_last_frame;
static uint64_t      nr_pdcp_current_time_last_subframe;
...
static void init_nr_rlc_data_req_queue(void)
{
  ...
  threadCreate(&t, rlc_data_req_thread,NULL,(char*)"RLC queue",-1,OAI_PRIORITY_RT);
}
...
static void init_nr_pdcp_data_ind_queue(void)
{
  ...
  threadCreate(&t, pdcp_data_ind_thread, NULL, (char*)"PDCP data ind", -1,OAI_PRIORITY_RT);
}
                                ...
                                char *buf, int size, int sdu_id)
{
  ...
  if (NODE_IS_CU(node_type)) {
    ...
    int qfi = -1;
    gtpv1uSendDirect(CUuniqInstance, ue_id, rb_id, (uint8_t *)buf, size, false, false, qfi);
  } else {
    uint8_t *memblock = malloc16(size);
    memcpy(memblock, buf, size);
             ...
             bool Ho_Tx)
{
  ...
  nr_pdcp_manager_lock(nr_pdcp_ue_manager);
  ue = nr_pdcp_manager_get_ue(nr_pdcp_ue_manager, UEid);
  if (nr_pdcp_get_rb(ue, srb_id, true) != NULL || Ho_Tx) {
    pdcp_srb = new_nr_pdcp_entity(NR_PDCP_SRB,
                                  ...
                                  -1,
                                  security_parameters,
                                  rrc->dl_srb_count_target);
    nr_pdcp_ue_add_srb_pdcp_entity(ue, srb_id, pdcp_srb, Ho_Tx);
  } else {
    pdcp_srb = new_nr_pdcp_entity(NR_PDCP_SRB,
                                  is_gnb,
                                  srb_id,
                                  ...
                                  security_parameters,
                                  rrc->dl_srb_count_target);
    nr_pdcp_ue_add_srb_pdcp_entity(ue, srb_id, pdcp_srb, false);
                      ...
                      bool Ho_Tx)
{
  if (srb2add_list != NULL) {
    for (int i = 0; i < srb2add_list->list.count; i++) {
      add_srb(enb_flag, UEid, srb2add_list->list.array[i], security_parameters, Ho_Tx);
    }
  } else
...
/* N2 HO Proprietary Change: interop TBD */
  gNB_RRC_INST *rrc = RC.nrrrc[0];
  rrc->dl_srb_count_source = rb->tx_next;
#endif
  ...
  if (pdu_size == -1) {
    nr_pdcp_manager_unlock(nr_pdcp_ue_manager);
    if (rb->do_drop) {
      LOG_D(PDCP, " srb - rlc tx buffer for (ue_id %ld, rb_id %ld) rlc free = %d,rlc occupied = %d \n", ue_id, rb_id,nr_rlc_get_available_tx_space(ue_id, rb_id),rlc_data_queue_occupancy());
      rb->rlc_tx_freesize = nr_rlc_get_available_tx_space(ue_id, rb_id) - rlc_data_queue_occupancy();
      if (rb->rlc_tx_freesize < 0) rb->rlc_tx_freesize = 0;
    }
                          ...
                          const uint32_t *const destinationL2Id)
{
  ...
  if (pdu_size == -1) {
    nr_pdcp_manager_unlock(nr_pdcp_ue_manager);
    if (rb->do_drop) {
      LOG_D(PDCP, " drb - rlc tx buffer for (ue_id %ld, rb_id %ld) rlc free = %d,rlc occupied = %d \n", ue_id, rb_id+3,nr_rlc_get_available_tx_space(ue_id, rb_id+3),rlc_data_queue_occupancy());
      rb->rlc_tx_freesize = nr_rlc_get_available_tx_space(ue_id, rb_id+3) - rlc_data_queue_occupancy();
      if (rb->rlc_tx_freesize < 0) rb->rlc_tx_freesize = 0;
    }
    /* update stats of dropped SDUs */
    rb->dropped_sdus++;
