...
void nr_csi_meas_reporting(int Mod_idP,frame_t frame, sub_frame_t slot)
{
  ...
  UE_iterator(nrmac->UE_info.list, UE ) {
    ...
    for (int csi_report_id = 0; csi_report_id < csi_measconfig->csi_ReportConfigToAddModList->list.count; csi_report_id++){
      ...
      #ifdef UNI_RAN
      if (res_index >= n) {
        LOG_W(PHY, "CSI pucch resource %ld not found among PUCCH resources. Skipping scheduling.\n", pucchcsires->pucch_Resource);
        return;
      }
      #else
      ...
      NR_sched_pucch_t *curr_pucch = &sched_ctrl->sched_pucch[pucch_index];
      #ifdef UNI_RAN
      // Check if the current PUCCH is already active (scheduled in advance)
      if (curr_pucch->active) {
        LOG_W(PHY, "CSI structure is scheduled in advance at %d.%d. Skipping scheduling.\n", sched_frame, sched_slot);
        return;
                                ...
                                uint8_t cqi_Table)
{
  ...
  if (cqi_bitlen > 4) {
    ...
    LOG_I(MAC,"Wide-band CQI for the second TB %d\n", temp_cqi);
  }

  if (UE->avg_cqi == 0 ) UE->avg_cqi = 15; 
  ...
  if (cqi_idx > UE->avg_cqi) UE->avg_cqi = cqi_idx;
  if ((UE->avg_cqi > 9) && (UE->avg_cqi - cqi_idx) > 3) UE->avg_cqi--;

  sched_ctrl->dl_max_mcs = get_mcs_from_cqi(mcs_table, cqi_Table, UE->avg_cqi);
  sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.wb_cqi_1tb = UE->avg_cqi;
  LOG_D(MAC,"MCS selected %d input cqi %d avg cqi %d \n", sched_ctrl->dl_max_mcs,cqi_idx,UE->avg_cqi);
...
#ifdef UNI_RAN
static inline int time_diff(frame_t f1, sub_frame_t s1, frame_t f2, sub_frame_t s2) {
  // Returns: + if (f1.s1) > (f2.s2), 0 if equal, - if (f1.s1) < (f2.s2)
  int frame_diff = (f1 - f2 + 1024) % 1024;
  if (frame_diff > 512) frame_diff -= 1024; // Wrap-around adjustment
  int slot_diff = s1 - s2;
  return (frame_diff != 0) ? frame_diff : slot_diff;
}
                                ...
                                int harq_round_max)
{
  ...
  while (time_diff(harq->feedback_frame, harq->feedback_slot, frame, slot) < 0) {
    LOG_W(NR_MAC,
          "UE %04x expected HARQ pid %d feedback at %4d.%2d, but is at %4d.%2d instead (HARQ feedback is in the past)\n",
          rnti, pid,
          harq->feedback_frame, harq->feedback_slot,
          frame, slot);

    // Audit and deactivate stale PUCCH
    for (int i = 0; i < sched_ctrl->sched_pucch_size; i++) {
      NR_sched_pucch_t *pucch = &sched_ctrl->sched_pucch[i];
      if (pucch->frame == harq->feedback_frame && pucch->ul_slot == harq->feedback_slot) {
        LOG_I(NR_MAC, "%s():  Audit - PUCCH Reset %d - %d \n", __func__, pucch->frame, pucch->ul_slot);
        pucch->active = 0;
      }
    }
    ...
    if (pid < 0 || ++loop_guard > harq_round_max)
      return NULL;

    harq = &sched_ctrl->harq_processes[pid];
  }

  if (time_diff(harq->feedback_frame, harq->feedback_slot, frame, slot) > 0) {
    LOG_W(NR_MAC,
