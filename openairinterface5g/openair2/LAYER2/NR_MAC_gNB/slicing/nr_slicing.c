               ...
               sub_frame_t slot)
{
  ...
  for (int i = 0; i < si->num; ++i) {
    ...
    if (p->type == NVS_RATE) {
      ...
      float inst = (float)ip->rb / n_rb_sched[0];
  ...
  if (maxidx < 0)
    return;
  ((_nvs_int_t *)si->s[maxidx]->int_data)->rb = n_rb_sched[0];

  UE_iterator (si->s[maxidx]->UE_list, UE1) {
    UE1->UE_sched_ctrl.last_sched_slice = maxidx;
  }
  ...
  LOG_D(NR_MAC, "%4d.%2d schedule %d RBs at slice idx %d ID %d \n", frame, slot, n_rb_sched[0] - rb_rem, maxidx, si->s[maxidx]->id);

  if (rb_rem == n_rb_sched[0]) // if no RBs have been used mark as inactive
