...
#ifdef UNI_RAN
static void nr_fill_nfapi_dl_PAGING_pdu(int Mod_idP,
                                        ...
                                        int beam_index)
{
  /* Set data according to SIB type (SIB1/OtherSI) */
  ...
  NR_ServingCellConfigCommon_t *scc = cc->ServingCellConfigCommon;
  pdsch->time_domain_allocation = 2;
  default_table_type_t table_type = get_default_table_type(1);//(type0_PDCCH_CSS_config->type0_pdcch_ss_mux_pattern);
  NR_tda_info_t tda_info = get_info_from_tda_tables(table_type,
                                                    pdsch->time_domain_allocation,
                                                    0, /*dmrs_TypeA_Position*/
                                                    true /*normal CP*/);
  tda_info.startSymbolIndex = 1;
  tda_info.nrOfSymbols = 5;
  tda_info.k2 = 0;
  pdsch->tda_info = tda_info;
  pdsch->tb_size = TBS;
  pdsch->dmrs_parms.dmrsConfigType        = NFAPI_NR_DMRS_TYPE1;
  pdsch->dmrs_parms.dl_dmrs_symb_pos      = 0x0004; /*DMRS Symbol POS ==  2*/
  pdsch->dmrs_parms.numDmrsCdmGrpsNoData  = 2;
  pdsch->dmrs_parms.N_PRB_DMRS            = 6 * pdsch->dmrs_parms.numDmrsCdmGrpsNoData;
  pdsch->dmrs_parms.N_DMRS_SLOT           = 1;   /*Number of DMRS symbols in the slot (2->1)*/
  pdsch->dmrs_parms.dmrs_ports_id         = 0;
  ...
  LOG_D(NR_MAC,
        "Paging :bwpStart %d, bwpSize %d, rbStart %d, rbSize %d, dlDmrsSymbPos = 0x%x\n",
  ...
  dci_payload.short_messages_indicator = 1;
  ...
  const int rnti_type = TYPE_P_RNTI_;
  ...
  LOG_D(MAC,
        "Paging: BWPSize: %3i, BWPStart: %3i, SubcarrierSpacing: %i, CyclicPrefix: %i, StartSymbolIndex: %i, DurationSymbols: %i, "
...
#ifdef UNI_RAN
void schedule_nr_paging(module_id_t module_idP,
                        frame_t frameP,
                        sub_frame_t slotP,
                        nfapi_nr_dl_tti_request_t *DL_req,
                        nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  if(gNB_mac->freeUeIndexPaging < 0) {
    return;
  } else {
    free_ue_index_paging = gNB_mac->freeUeIndexPaging;
  }
  int L_max;
  switch (scc->ssb_PositionsInBurst->present) {
    case 1:
  ...
  /* quick checks: do we even have pending paging to schedule? */
  if (!gNB_mac->pcch_pending || gNB_mac->pcch_pdu_size <= 0) return;

  /* For each SSB entry (type0 CSS config), check whether we should transmit paging here */
  for (int i = 0; i < L_max; i++) {

    NR_Type0_PDCCH_CSS_config_t *type0_PDCCH_CSS_config = &gNB_mac->type0_PDCCH_CSS_config[i];

    /* basic activation / slot/frame check (same as SIB1) */

    if (!(type0_PDCCH_CSS_config->active && type0_PDCCH_CSS_config->num_rbs > 0))
      continue;

    /* get PF/PO/T from stored UE list */
    uint16_t PF_min = NR_UE_PF_PO[CC_id][free_ue_index_paging].PF_min; /* 0..T-1 */
    uint8_t PO = NR_UE_PF_PO[CC_id][free_ue_index_paging].PO;          /* slot in PF */
    uint16_t T = NR_UE_PF_PO[CC_id][free_ue_index_paging].T;
    bool pf_match = ((frameP % T) == PF_min);
    bool po_match = (slotP == PO);

    if (!(pf_match && po_match))
        continue;
    if (pf_match && po_match){
       LOG_D(NR_MAC, "Paging to be scheduled for UE[%d]: frameP = %d, slotP = %d\n", free_ue_index_paging+1, frameP, slotP);
    }

    /* Passed timing checks -> schedule paging using this type0 CSS */
    const int n_slots_frame = gNB_mac->frame_structure.numb_slots_frame;
    int beam_index = get_fapi_beamforming_index(gNB_mac, i);
    NR_beam_alloc_t beam = beam_allocation_procedure(&gNB_mac->beam_info, frameP, slotP, beam_index, n_slots_frame);
    AssertFatal(beam.idx >= 0, "Cannot allocate paging beam for SSB %d\n", i);
    LOG_D(NR_MAC, "(%d.%d) Paging transmission: ssb_index %d\n", frameP, slotP, type0_PDCCH_CSS_config->ssb_index);
    ...
    NR_tda_info_t tda_info = get_info_from_tda_tables(table_type,
                                                      ...
                                                      0,/*dmrs_TypeA_Position*/
                                                      true /*normal CP*/);
    /*Setting TDA INFO as per TDA index - 2 indicated in SIB1*/
    tda_info.startSymbolIndex = 1;
    tda_info.nrOfSymbols = 5;
    tda_info.k2 = 0;
    tda_info.mapping_type = 1;
    tda_info.valid_tda = true;

    AssertFatal((tda_info.startSymbolIndex + tda_info.nrOfSymbols) < 14,
                "Paging TDA %d would overlap with symbols. Pick a different TDA.\n",
                time_domain_allocation);

    /* Getting DMRS parameters for paging */
    NR_pdsch_dmrs_t dmrs_parms = get_dl_dmrs_params(scc, NULL, &tda_info, 1);
    dmrs_parms.dl_dmrs_symb_pos = 0x0004;
    dmrs_parms.dmrsConfigType = NFAPI_NR_DMRS_TYPE1;
    dmrs_parms.numDmrsCdmGrpsNoData = 2;
    dmrs_parms.N_PRB_DMRS           = 6 * dmrs_parms.numDmrsCdmGrpsNoData;
    dmrs_parms.N_DMRS_SLOT          = 1;
    dmrs_parms.dmrs_ports_id       = 0;
    ...
    uint32_t TBS = schedule_control_sib1(module_idP, CC_id,
                                         ...
                                         candidate_idx,
                                         beam.idx,
                                         gNB_mac->pcch_pdu_size + 2/* Setting 2 extra bytes wrt paging payload to get enough RBs */);
    /* Build DL TTI PDUs (PDCCH + PDSCH) and TX request.
       Using the PCCH payload and paging helper to fill the PDU. */

    nfapi_nr_dl_tti_request_body_t *dl_req = &DL_req->dl_tti_request_body;
    int pdu_index = gNB_mac->pdu_index[0]++;
    LOG_D(NR_MAC, "%s() %4d.%2d scheduling paging\n", __func__, frameP, slotP);

    /* Fill NFAPI DL PDCCH+PDSCH for paging. This helper should prepare PDCCH to send a DCI with P-RNTI
       and PDSCH configured for the PCCH payload (gNB_mac->pcch_pdu, size gNB_mac->pcch_pdu_size) */
    ...
    /* Attach payload to TX_req */
    const int ntx_req = TX_req->Number_of_PDUs;
    nfapi_nr_pdu_t *tx_req = &TX_req->pdu_list[ntx_req];
    memset(tx_req, 0, sizeof(*tx_req));

    if (gNB_mac->pcch_pdu_size > 0) {
      memcpy(tx_req->TLVs[0].value.direct, gNB_mac->pcch_pdu, TBS);
    }
    ...
    LOG_I(NR_MAC, "[PCCH] Scheduling paging for UE[%d] (size=%d, TBS = %32u) at %d.%d, ssb=%d, pduIndex=%d\n",
          free_ue_index_paging+1, gNB_mac->pcch_pdu_size, TBS, frameP, slotP, type0_PDCCH_CSS_config->ssb_index, pdu_index);

    /* mark pcch consumed */
    gNB_mac->pcch_pending  = false;
    gNB_mac->pcch_pdu_size = 0;
    type0_PDCCH_CSS_config->active = false;
    LOG_D(NR_MAC, "[PCCH] Scheduled  and sent to PHY for UE[%d] (size=%d, TBS = %32u) at %d.%d, ssb=%d, pduIndex=%d successfully!!\n",
          free_ue_index_paging+1, gNB_mac->pcch_pdu_size, TBS, frameP, slotP, type0_PDCCH_CSS_config->ssb_index, pdu_index);
