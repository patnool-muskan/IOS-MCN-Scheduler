...
typedef struct {
  atomic_ulong nr_rlc_current_time;
  atomic_ulong nr_rlc_current_time_last_frame;
  atomic_ulong nr_rlc_current_time_last_subframe;
} nr_rlc_time_t;

nr_rlc_time_t nr_rlc_last;

static uint64_t get_nr_rlc_current_time(void)
{
  return nr_rlc_last.nr_rlc_current_time;
}

void nr_rlc_tick(int frame, int subframe)
{
  nr_rlc_time_t nr_rlc_current = {
    .nr_rlc_current_time = get_nr_rlc_current_time()+1,
    .nr_rlc_current_time_last_frame = frame,
    .nr_rlc_current_time_last_subframe = subframe,
  };
  atomic_compare_exchange_strong(&nr_rlc_current,&nr_rlc_last,nr_rlc_current);
}
#elif
...
{
  LOG_D(RLC, "release enity with LCID %d\n", channel_id);
...
static nr_rlc_entity_t *get_rlc_entity_from_lcid(nr_rlc_ue_t *ue, logical_chan_id_t channel_id)
{
  ...
  nr_rlc_rb_t *rb = &ue->lcid2rb[channel_id - 1];
  if (rb->type == NR_RLC_NONE){
    LOG_D(RLC, "RB type NULL on LCID %d\n", channel_id);
    return NULL;
  }
  if (rb->type == NR_RLC_SRB) {
...
/**
 * @brief This function discards all stored RLC PDUs (TX)
 * @param ue the target RLC entity
 */
void nr_rlc_discard_all_pdus(int ue_id, bool is_srb){
  nr_rlc_manager_lock(nr_rlc_ue_manager);
  nr_rlc_ue_t *ue = nr_rlc_manager_get_ue(nr_rlc_ue_manager, ue_id);
  if (is_srb) {
    for (int i = 1; i < sizeof(ue->srb) / sizeof(ue->srb[0]); i++) {
      if (ue->srb[i]) {
        LOG_I(RLC, "%s: discard PDUs for SRB %d \n", __func__, i + 1);
        ue->srb[i]->discard_pdus(ue->srb[i]);
      }
    }
  } else {
    for (int i = 0; i < sizeof(ue->drb) / sizeof(ue->drb[0]); i++) {
      if (ue->drb[i]) {
        LOG_I(RLC, "%s: discard PDUs for DRB %d \n", __func__, i + 1);
        ue->drb[i]->discard_pdus(ue->drb[i]);
      }
    }
  }
  nr_rlc_manager_unlock(nr_rlc_ue_manager);
...
rb_found:
  ...
  if (is_enb) {
	...
	extern instance_t DUuniqInstance;
        int qfi = -1;
        gtpv1uSendDirect(DUuniqInstance, ue->ue_id, rb_id, (uint8_t*) buf, size, false, false, qfi);
	return;
      }
    }
  }
...
bool is_rlc_ue_available(const int ue_id) {
    nr_rlc_ue_manager_internal_t *m = nr_rlc_ue_manager;
    bool ret_val = false;
