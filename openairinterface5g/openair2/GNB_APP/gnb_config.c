...
static NR_ServingCellConfigCommon_t *get_scc_config(configmodule_interface_t *cfg, int minRXTXTIME)
{
  ...
  #ifdef UNI_RAN
  asn1cCallocOne(pcc->pagingSearchSpace, 0);
  #else
...
f1ap_fdd_info_t read_fdd_config(const NR_ServingCellConfigCommon_t *scc)
{
  const NR_FrequencyInfoDL_t *dl = scc->downlinkConfigCommon->frequencyInfoDL;
...
static void fill_measurement_configuration(uint8_t gnb_idx, gNB_RRC_INST *rrc)
{
  ...
  LOG_I(GNB_APP, "HO LOG: A3 Configuration Exists: %d\n", A3_EventList.numelt);
...
#ifdef UNI_RAN
    rrc->inactivity_timer_threshold = *(GNBParamList.paramarray[i][GNB_RRC_INACTIVITY_TIMER].u64ptr);
    rrc->inactivity_interval_sec = *(GNBParamList.paramarray[i][GNB_INACTIVITY_INTERVAL_SEC].uptr);
    rrc->inactivity_interval_us = *(GNBParamList.paramarray[i][GNB_INACTIVITY_INTERVAL_US].uptr);
    LOG_I(RRC, "Inactivity Config Init: Threshold = % " PRIu64 ", Interval sec = %u" ", Interval us = %u""\n", rrc->inactivity_timer_threshold, rrc->inactivity_interval_sec, rrc->inactivity_interval_us);
#endif
    sprintf(aprefix, "%s.[%i]", GNB_CONFIG_STRING_GNB_LIST, 0);
...
#ifdef UNI_RAN
  rrc->pdcp_config.drb.sn_size = 18; /* TBD: should be used as #define */
#endif
