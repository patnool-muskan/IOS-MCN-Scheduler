...
/* @brief xran-specific callback, called when all packets for given CC and
 * symbol arrived (see * xran_5g_fronthault_config()).
 *
 * Unblocks xran_fh_rx_read_slot()  */
...
/* @brief xran-specific callback for time alignment, see xran_reg_physide_cb().
 * Only used to unblock timing in oai_xran_fh_rx_callback() on first call. */
...
/* @brief Read PRACH data from xran buffers, optionally uncompress, and place
 * into OAI buffers. */
...
/* @brief check if symbol in slot slot is UL. */
...
/* @brief Read PRACH and PUSCH data from xran buffers, optionally
 * decompressing. Print some counters every 128 frames.
 *
 * Function is blocking and waits for next frame/slot combination. Is unblocked
...
/* @brief Write PDSCH IQ-data to xran buffers, optionally compressing data. */
