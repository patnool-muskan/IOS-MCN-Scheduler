...
static bool set_fh_io_cfg(struct xran_io_cfg *io_cfg, const paramdef_t *fhip, int nump, const int num_rus)
{
  ...
  // - system core added
  io_cfg->pkt_aux_core = io_cfg->system_core; // sapmle app says 0 = "do not start" 
  io_cfg->io_sleep = 1; // no sleep
  ...
  io_cfg->io_sleep = 1; // enable sleep on PMD cores; 0 -> no sleep
...
static bool set_fh_eaxcid_conf_mplane(struct xran_eaxcid_config *eaxcid_conf, enum xran_category cat, const ru_session_list_t *ru_session_list)
{
  ...
  switch (cat) {
    case XRAN_CATEGORY_A:
      ...
      eaxcid_conf->bit_bandSectorId = xran_mplane->band_sector; // total number of band sectors supported by O-RU should be retreived by M-plane - <max-num-bands> && <max-num-sectors>
      eaxcid_conf->bit_ccId = xran_mplane->ccid; // total number of CC supported by O-RU should be retreived by M-plane - <max-num-component-carriers>
...
static bool set_fh_init(void *mplane_api, struct xran_fh_init *fh_init, enum xran_category xran_cat)
{
  ...
  if(mplane_enabled){
    ...
    //   }
    // }
//   Temporarily adding some changes to check the issues
...
// End of temporary additions // Safwan
  }else {
                                ...
                                struct xran_prach_config *prach_config)
{
  ...
  //-  For GEN 1 VVDN radio
  // prach_config->nPrachFreqOffset = 0;
  /* 
  if (oai0->nr_band < 100)
...
/* 
  ...
  //Hard coded For GEN1 VVDN Radio
  //fh_config->nDLFftSize = 4096;
  //fh_config->nULFftSize = 4096;
  //Hard coded For GEN1 VVDN Radio
  //fh_config->nDLAbsFrePointA = 3650880;
  //fh_config->nULAbsFrePointA = 3650880;
