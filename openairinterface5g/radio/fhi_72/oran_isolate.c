...
{
  printf("1. openair0_cfg->split7.fftSize %d\n", openair0_cfg->split7.fftSize);
  ...
  printf("2. openair0_cfg->split7.fftSize %d\n", openair0_cfg->split7.fftSize);
  bool success = false;
  if(mplane_enabled) {
    ...
    for (size_t i = 0; i < ru_session_list.num_rus; i++) {
      ...
      ru_configured[i] = connect_mplane(ru_session, ru_session_list.du_key_pair, ru_session_list.ru_session[0].ru_username);
      if (!ru_configured[i]) {
        continue;
      }
