...
bool init_mplane(ru_session_list_t *ru_session_list)
{
  ...
  AssertError(num_keys == 2, return false, "[MPLANE] Expected du_key_pair = ({pub-key-path, priv-key-path}) in configuration file.\n");

  char **ru_ip_addrs = gpd(fhip, nump, ORAN_CONFIG_RU_IP_ADDR)->strlistptr;
  char **ru_username = gpd(fhip, nump, ORAN_CONFIG_RU_USERNAME)->strlistptr;
  ...
  if(num_rus == 0) {
    if(ru_session_list->ru_session == NULL) {
      ...
      listen_mplane(&ru_session_list->ru_session, ru_session_list->du_key_pair, ru_username);
    }
  } else {
      ru_session_list->num_rus = num_rus;
      ru_session_list->ru_session = calloc(num_rus, sizeof(ru_session_t));
      for (size_t i = 0; i< num_rus; i++) {
        ...
        // allocate memory for the RU session username
        ru_session->ru_username = calloc(strlen(ru_username[i]) + 1, sizeof(char));
        memcpy(ru_session->ru_username, ru_username[i], strlen(ru_username[i]) + 1);
        // allocate memory for the RU IP address
...
bool manage_ru(ru_session_t *ru_session, const openair0_config_t *oai, const size_t num_rus)
{
  ...
  AssertError(success, return false, "[MPLANE] Unable to retreive required info for xran from RU \"%s\".\n", ru_session->ru_ip_add);
