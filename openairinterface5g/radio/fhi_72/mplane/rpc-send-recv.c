...
static bool recv_v2(struct nc_session *session, struct nc_rpc *rpc, NC_MSG_TYPE msgtype, const uint64_t msgid, const NC_WD_MODE wd_mode, const int timeout_ms, char **answer)
{
  ...
  /* edit/validate/commit functionalities */
    ...
    LY_LIST_FOR(lyd_child(envp), err) {
      struct lyd_node *type, *tag, *severity, *app_tag, *path, *msg, *info = NULL;
      ...
      lyd_find_sibling_opaq_next(lyd_child(err), "error-message", &msg);
      ...
      MP_LOG_I("RPC reply = ERROR\n\
                ...
                \tpath:     %s\n\
                \tmessage:  %s\n",
                ((struct lyd_node_opaq *)type)->value,
                ((struct lyd_node_opaq *)tag)->value,
                ((struct lyd_node_opaq *)severity)->value,
                ((struct lyd_node_opaq *)app_tag)->value,
                ((struct lyd_node_opaq *)path)->value,
                ((struct lyd_node_opaq *)msg)->value);
