...
#ifdef MPLANE_V2
static bool create_cu_interface_v2(struct ly_ctx **ctx, const ru_session_t *ru_session, struct lyd_node **root)
{
  ...
  char vlan_tag_str[8];
  sprintf(vlan_tag_str, "%d", ru_session->ru_mplane_config.vlan_tag[0]); // assuming only 1 ethernet interface is allowed for both CU-planes
...
static bool create_proc_elem_v2(struct ly_ctx **ctx, const ru_session_t *ru_session, struct lyd_node **root)
{
  LY_ERR ret = LY_SUCCESS;
...
static bool create_uplane_conf_v2(struct ly_ctx **ctx, const ru_session_t *ru_session, const openair0_config_t *oai, const size_t num_rus, struct lyd_node **root)
{
  LY_ERR ret = LY_SUCCESS;
  bool success = true;
