...
#define ATR_TX   0xC  //data[3] and data[2]
...
static void trx_usrp_start_generic_gpio(openair0_device *device, usrp_state_t *s)
{
  ...
  s->usrp->set_gpio_attr(s->gpio_bank, "ATR_TX", ATR_TX, ATR_MASK);
...
static void trx_usrp_finish_rx(usrp_state_t *s)
{
  ...
  size_t samples;
  uint8_t buf[1024];
  double timeout = 0.00005;
  std::vector<void *> buff_ptrs;
  for (size_t i = 0; i < s->usrp->get_rx_num_channels(); i++) buff_ptrs.push_back(buf);
  do {
    samples = s->rx_stream->recv(buff_ptrs, sizeof(buf)/4, s->rx_md,timeout);
  } while (samples > 0);
}
...
void *trx_usrp_write_thread(void * arg){
  ...
  while(1){
    ...
    double tx_delay= 0.000000;
    const double burst_pkt_time = std::max<double>(0.100f,(2.0 *nsamps / s->sample_rate));
    ...
    if (cc>1) {
      ...
      ret = (int)s->tx_stream->send(buff_ptrs, nsamps, s->tx_md,timeout);
    }
    else {
      ret = (int)s->tx_stream->send(&(((int16_t *)buff_tx[0])[0]), nsamps, s->tx_md,timeout);
    }
...
int trx_usrp_write_init(openair0_device *device){
  ...
  threadCreate(&write_thread->pthread_write,
               ...
               (void *)device,
               (char*)"tx_wr_thread",
               device->openair0_cfg->txfh_cores[0],
               OAI_PRIORITY_RT_MAX);
  return(0);
}
...
static int trx_usrp_read(openair0_device *device, openair0_timestamp *ptimestamp, void **buff, int nsamps, int cc) {
  ...
  samples_received=0;
  double rx_trans_time = nsamps / s->sample_rate;
  double timeout = std::max<double>(0.1f,(2.0 * rx_trans_time));
  while (samples_received != nsamps) {
    ...
    if (cc>1) {
      ...
      samples_received += s->rx_stream->recv(buff_ptrs, nsamps-samples_received, s->rx_md,timeout);
    } else {
      // receive a single channel (e.g. from connector RF A)

      samples_received += s->rx_stream->recv((void*)((int32_t*)buff_tmp[0]+samples_received),
                                             nsamps-samples_received, s->rx_md,timeout);
    }
...
extern "C" {
  int device_init(openair0_device *device, openair0_config_t *openair0_cfg) {
    ...
    std::string args,tx_subdev,rx_subdev,rx_ant,tx_ant;
  ...
  if(openair0_cfg[0].rx_ant!=NULL){
    rx_ant = openair0_cfg[0].rx_ant;
    s->usrp->set_rx_antenna(rx_ant,i);
  }
  if(openair0_cfg[0].tx_ant!=NULL){
    tx_ant = openair0_cfg[0].tx_ant;
    s->usrp->set_tx_antenna(tx_ant,i);
  }
  }
  for(int i=0; i<((int) s->usrp->get_rx_num_channels()); i++) {
