...
static void NRRCconfig_RU(configmodule_interface_t *cfg)
{
  ...
  if (RUParamList.numelt > 0) {
    ...
    for (int j = 0; j < RC.nb_RU; j++) {
      ...
      if (config_isparamset(RUParamList.paramarray[j], RU_TX_ANT)) {
        RC.ru[j]->openair0_cfg.tx_ant = strdup(*(RUParamList.paramarray[j][RU_TX_ANT].strptr));
        LOG_I(PHY, "RU USRP tx ant == %s\n", RC.ru[j]->openair0_cfg.tx_ant);
      }

      if (config_isparamset(RUParamList.paramarray[j], RU_RX_ANT)) {
        RC.ru[j]->openair0_cfg.rx_ant = strdup(*(RUParamList.paramarray[j][RU_RX_ANT].strptr));
        LOG_I(PHY, "RU USRP rx ant == %s\n", RC.ru[j]->openair0_cfg.rx_ant);
      }
