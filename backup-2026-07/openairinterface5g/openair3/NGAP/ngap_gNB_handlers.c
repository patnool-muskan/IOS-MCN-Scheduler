...
{
  NGAP_INFO("HO LOG: Handover Request Message Has Came to the Target gNB\n");
  uint64_t                      amf_ue_ngap_id;
...
static int ngap_gNB_handle_pdusession_setup_request(sctp_assoc_t assoc_id, uint32_t stream, NGAP_NGAP_PDU_t *pdu)
{
  ...
  //                        IOS-MCN UNI Changes for 2nd pdu session, When UEAggregateMaximumBitRat is absent
  if ( ie != NULL) {
    asn_INTEGER2ulong(&(ie->value.choice.UEAggregateMaximumBitRate.uEAggregateMaximumBitRateUL),
    ...
    /* default value added by IOS-MCN UNI */
    msg->ueAggMaxBitRateDownlink = 1000000000; //1 Gbps
    msg->ueAggMaxBitRateUplink = 150000000; //150 Mbps
  }
  ...
  for (int i = 0; i < ie->value.choice.PDUSessionResourceSetupListSUReq.list.count; i++) {
    ...
    if (item_p->pDUSessionNAS_PDU) // Added by IOS-MCN UNI
...
static int ngap_gNB_handle_paging(sctp_assoc_t assoc_id, uint32_t stream, NGAP_NGAP_PDU_t *pdu)
{
  ...
  if (stream == 0) {
    LOG_D(NGAP,"[SCTP %d] Received Paging procedure on stream (%d) and ready for RRC Layer\n",
          assoc_id, stream);
    #ifndef UNI_RAN
    return -1;
    #endif
  ...
  if (ngap_gNB_instance == NULL) {
   ...
   NGAP_DEBUG("[SCTP %u] Received Paging Identity amf_set_id %d, amf_pointer %d, m_tmsi %u\n",
              assoc_id,
  ...
  for (int i = 0; i < ie->value.choice.TAIListForPaging.list.count; i++) {
    ...
    OCTET_STRING_TO_INT24(&(item_p->tAI.tAC), msg->tac[i]);
    msg->tai_size++;
  ...
  NGAP_DEBUG("[SCTP %u] Received Paging parameters: Paging Identity amf_set_id %d amf_pointer %d m_tmsi %u paging_drx %d paging_priority %d\n",assoc_id,
             msg->ue_paging_identity.s_tmsi.amf_set_id,
             msg->ue_paging_identity.s_tmsi.amf_pointer,
             msg->ue_paging_identity.s_tmsi.m_tmsi,
