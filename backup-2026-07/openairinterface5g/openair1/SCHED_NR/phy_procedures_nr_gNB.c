...
#ifdef UNI_RAN
#define PUSCH_THRES_MARGIN_X10 (-20)
#define PUSCH_THRES_MIN_X10 (-80)  /* −8 dB (QPSK edge) */
#define PUSCH_THRES_MAX_X10 (-20)  /* Should be 16 dB (64QAM ceiling) ---> -20 dB Temporary */
#endif
  ...
  for (int i = 0; i < NR_SYMBOLS_PER_SLOT; i++){
    ...
    if (csirs->active == 1) {
      ...
      nr_generate_csi_rs(&gNB->frame_parms,
                         (c16_t **) gNB->common_vars.txdataF[beam_nb],
                         gNB->TX_AMP,
                         csi_params,
                         slot,
                         &mapping_parms);
...
int phy_procedures_gNB_uespec_RX(PHY_VARS_gNB *gNB, int frame_rx, int slot_rx, NR_UL_IND_t *UL_INFO)
{
  ...
  for (int ULSCH_id = 0; ULSCH_id < gNB->max_nb_pusch; ULSCH_id++) {
    ...
    if ((ulsch->active == true) && (ulsch->frame == frame_rx) && (ulsch->slot == slot_rx) && (ulsch->handled == 0)) {
      ...
      #ifdef UNI_RAN
      /*-------------------------------------------------------------------------------------
      ------------------DYNAMIC PUSCH Threshold adjustment - Temporary Fix-------------------
      -------------------------------------------------------------------------------------*/
      int32_t snr_x10 = dB_fixed_x10(pusch_vars->ulsch_power_tot) - dB_fixed_x10(pusch_vars->ulsch_noise_power_tot);
      NR_gNB_PHY_STATS_t *stats_data = get_phy_stats(gNB, ulsch->rnti);
      if (stats_data && stats_data->pusch_snr_init != 0 && stats_data->pusch_snr_init != 1) {
        stats_data->pusch_snr_init = 0;
        stats_data->avg_pusch_snr_x10 = 0;
      }
      if (stats_data) {
        if (!stats_data->pusch_snr_init) {
        stats_data->avg_pusch_snr_x10 = snr_x10;
        stats_data->pusch_snr_init = 1;
        } else {
        /* Too Fast IIR filter (1/2) */
        stats_data->avg_pusch_snr_x10 = (1 * stats_data->avg_pusch_snr_x10 + snr_x10) >> 1;
        }
      }

      if (stats_data) {
        int32_t dyn_thres = stats_data->avg_pusch_snr_x10;
        if (dyn_thres < PUSCH_THRES_MIN_X10){
          dyn_thres = PUSCH_THRES_MIN_X10 + PUSCH_THRES_MARGIN_X10;
        }
        if (dyn_thres > PUSCH_THRES_MAX_X10){
          dyn_thres = PUSCH_THRES_MAX_X10 + PUSCH_THRES_MARGIN_X10;
        }

        gNB->pusch_thres = dyn_thres;
