...
void nr_fill_pucch(PHY_VARS_gNB *gNB, int frame, int slot, nfapi_nr_pucch_pdu_t *pucch_pdu)
{
  ...
  #ifdef UNI_RAN
  if (!found) {
        LOG_W(PHY, "No available PUCCH resource for %d.%d. Skipping scheduling.\n", frame, slot);
        return;
  };
  #else
  // Resource Audit
  if (!found) {
  	for (int i = 0; i < gNB->max_nb_pucch; i++) {
		NR_gNB_PUCCH_t *pucch = &gNB->pucch[i];
		LOG_I(PHY, " Audit - Pucch not dalloc %d.%d ", pucch->frame,pucch->slot); 
	}
	LOG_I(PHY, "Pucch not dalloc \n"); 
  }
                      ...
                      nfapi_nr_pucch_pdu_t* pucch_pdu)
{
  ...
  for (int aa=0;aa<Prx;aa++){
    for (int symb=0;symb<nb_symbols;symb++) {
      ...
      pucch2_lev += signal_energy_nodc((c16_t*)rp[aa][symb],nb_re_pucch);
    }
  }
