...
int16_t find_nr_prach(PHY_VARS_gNB *gNB,int frame, int slot, find_type_t type) {
  ...
  for (uint16_t i=0; i<NUMBER_OF_NR_PRACH_MAX; i++) {
    ...
    // Resource Audit
    #ifdef UNI_RAN
    int f = gNB->prach_vars.list[i].frame;
    if (f>=0 && (frame -f + 1024)%1024 > 20) {
      LOG_D(PHY, "Auto-cleanup: freeing stale PRACH %d from frame %d\n", i, f);
      free_nr_prach_entry(gNB, i);
    }
    #else
    #ifdef RES_AUDIT 
    if (gNB->prach_vars.list[i].frame + 20 == frame) { 
      LOG_D(PHY," Audit - Freeing Prach %d \n", i);
      free_nr_prach_entry(gNB, i);
    } 
   #endif
   #endif
