...
#else
  uint32_t ue_rrc_inactivity_timer;
  uint16_t ue_rrc_srb_inactivity_timer;
  uint32_t ue_rrc_inactivity_threshold;
  uint32_t last_txsdu_pkts[MAX_DRBS_PER_UE];
  uint32_t last_rxsdu_pkts[MAX_DRBS_PER_UE];
  uint32_t last_txsdu_pkts_srb[NR_NUM_SRB];
  uint32_t last_rxsdu_pkts_srb[NR_NUM_SRB];
#endif
...
typedef struct NR_UE_PF_PO_s {
  ...
  uint32_t ue_index_value;  /* UE index value */
  uint16_t PF_min;  /* minimal value of Paging Frame (PF) */
...
extern NR_UE_PF_PO_t NR_UE_PF_PO[NFAPI_CC_MAX][MAX_MOBILES_PER_GNB];
extern pthread_mutex_t nr_ue_pf_po_mutex;
#endif
...
typedef struct gNB_RRC_INST_s {
...
#ifdef UNI_RAN
  uint64_t inactivity_timer_threshold;
  uint32_t inactivity_interval_sec;
  uint32_t inactivity_interval_us;
#endif
  // RRC configuration
  ...
  uint8_t dl_srb_count_source;
  uint8_t dl_srb_count_target;
#endif
} gNB_RRC_INST;
