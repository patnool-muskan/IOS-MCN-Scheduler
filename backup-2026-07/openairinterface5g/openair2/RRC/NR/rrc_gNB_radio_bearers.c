...
drb_t *find_drb_from_pduSession(gNB_RRC_UE_t *ue, int pdusession_id)
{
  drb_t *drb = NULL;
  DevAssert(ue != NULL);
  for (int i = 0; i < MAX_DRBS_PER_UE; i++) {
    drb = &ue->established_drbs[i];
    if (drb->status != DRB_INACTIVE && drb->cnAssociation.sdap_config.pdusession_id == pdusession_id)
      return drb;
  }
  return drb;
}
...
#ifdef UNI_RAN
drb_t *generateDRB_multiQos(gNB_RRC_UE_t *ue,
                            uint8_t drb_id,
                            const int pdusession_id,
                            bool enable_sdap,
                            int do_drb_integrity,
                            int do_drb_ciphering,
                            int nb_qos,
                            pdusession_level_qos_parameter_t *qos_list)
{
  DevAssert(ue != NULL);

  LOG_I(NR_RRC,
        "UE %d: configure DRB ID %d for PDU session ID %d with %d QoS flows\n",
        ue->rrc_ue_id, drb_id, pdusession_id, nb_qos);
  ...
  /* Clear mapped flows first */
  memset(est_drb->cnAssociation.sdap_config.mappedQoS_FlowsToAdd, 0, sizeof(est_drb->cnAssociation.sdap_config.mappedQoS_FlowsToAdd));

  /* Map multiple QoS flows */
  for (int i = 0; i < nb_qos; i++) {
    int qfi    = qos_list[i].qfi;
    int fiveQI = qos_list[i].fiveQI;

    est_drb->cnAssociation.sdap_config.mappedQoS_FlowsToAdd[i] = qfi;
    ...
    LOG_I(NR_RRC, "Mapped QFI %d (5QI %d) to DRB %d\n", qfi, fiveQI, drb_id);
  }
