...
static void xnap_gNB_ue_context_release(instance_t instance, xnap_ue_context_release_t *xnap_ue_context_release)
{
  ...
  /* free the XNAP UE ID */
  ue_id = xnap_id_get_cu_ueid(&instance_p->id_manager, xnap_ue_context_release->s_ng_node_ue_xnap_id);
  if (ue_id == -1) {
    exit(0);
  }
  ue_id = xnap_find_id_from_cu_ue_id(&instance_p->id_manager, ue_id);
  ...
  while (1) {
    ...
    switch (ITTI_MSG_ID(received_msg)) {
      ...
      case XNAP_SN_STATUS_TRANSFER: /* Xn code for SN Status transfer procedures */
        xnap_gNB_generate_xn_sn_status_transfer(ITTI_MSG_ORIGIN_INSTANCE(received_msg), &XNAP_SN_STATUS_TRANSFER(received_msg));
        break; 
