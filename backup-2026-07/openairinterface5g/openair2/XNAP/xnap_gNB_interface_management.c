...
int xnap_gNB_handle_ue_context_release(instance_t instance, sctp_assoc_t assoc_id, uint32_t stream, XNAP_XnAP_PDU_t *pdu)
{
  ...
  if (id_target != xnap_id_get_id_target(&instance_p->id_manager, ue_id)) {
    LOG_E(XNAP,
          ...
          xnap_id_get_cu_ueid(&instance_p->id_manager, ue_id),
          id_source,
  ...
  XNAP_UE_CONTEXT_RELEASE(msg).rnti = xnap_id_get_cu_ueid(&instance_p->id_manager, ue_id);
...
int xnap_gNB_handle_xn_sn_status_transfer (instance_t instance, sctp_assoc_t assoc_id, uint32_t stream, XNAP_XnAP_PDU_t *pdu)

{
  ...
  if(stream !=0)
        ...
        LOG_I(XNAP, "**[xnap_interface_management:handle_xn_sn_status_transfer] Received SN Status Transfer **\n");
        MessageDef *message_p = itti_alloc_new_message(TASK_XNAP, 0, XNAP_SN_STATUS_TRANSFER);
        message_p->ittiMsgHeader.originInstance = assoc_id;
        xnap_xn_sn_status_transfer_t *snstatustran = &XNAP_SN_STATUS_TRANSFER(message_p);
        ...
        return -1;
          ...
          printf ("Target NG_RANnodeUEXnAPID %lu\n", ie->value.choice.NG_RANnodeUEXnAPID);
        }


      // snstatustran->t_ng_node_ue_xnap_id = xn_id;  // must be received from handover_ack
      ...
      XNAP_FIND_PROTOCOLIE_BY_ID(XNAP_SNStatusTransfer_IEs_t, ie, handlesnstatustran, XNAP_ProtocolIE_ID_id_targetNG_RANnodeUEXnAPID, true);
        ...
        return -1;
          ...
          snstatustran->t_ng_node_ue_xnap_id = ie->value.choice.NG_RANnodeUEXnAPID;
          printf ("Target NG_RANnodeUEXnAPID %lu\n", ie->value.choice.NG_RANnodeUEXnAPID);
        }
       ...
       XNAP_FIND_PROTOCOLIE_BY_ID(XNAP_SNStatusTransfer_IEs_t, ie, handlesnstatustran, XNAP_ProtocolIE_ID_id_DRBsSubjectToStatusTransfer_List, true);
        if (ie != NULL) {
           snstatustran->drb_list.num_drb = ie->value.choice.DRBsSubjectToStatusTransfer_List.list.count; // need to update from the received
       printf ("IE Count %d\n", ie->value.choice.DRBsSubjectToStatusTransfer_List.list.count);
       for (int i = 0; i < ie->value.choice.DRBsSubjectToStatusTransfer_List.list.count; i++){
       XNAP_DRBsSubjectToStatusTransfer_Item_t *item_p = ie->value.choice.DRBsSubjectToStatusTransfer_List.list.array[i];
       snstatustran->drb_list.drb[i].drb_id = item_p->drbID;
       printf ("drbID : %ld\n", item_p->drbID);

       /* 12 bit */
       /* 
       snstatustran->drb_list.drb[i].pdcp_status_ul.pdcp_sn_12bit.count_value.pdcp_sn12 = item_p->pdcpStatusTransfer_UL.choice.pdcp_sn_12bits->cOUNTValue.pdcp_SN12;
       printf ("sn12 UL: %ld\n", item_p->pdcpStatusTransfer_UL.choice.pdcp_sn_12bits->cOUNTValue.pdcp_SN12);

       snstatustran->drb_list.drb[i].pdcp_status_ul.pdcp_sn_12bit.count_value.hfn_pdcp_sn12 = item_p->pdcpStatusTransfer_UL.choice.pdcp_sn_12bits->cOUNTValue.hfn_PDCP_SN12;
       printf ("hfn12 UL: %ld\n", item_p->pdcpStatusTransfer_UL.choice.pdcp_sn_12bits->cOUNTValue.hfn_PDCP_SN12);

       snstatustran->drb_list.drb[i].pdcp_status_dl.pdcp_sn_12bit.count_value.pdcp_sn12 = item_p->pdcpStatusTransfer_DL.choice.pdcp_sn_12bits->cOUNTValue.pdcp_SN12;
       printf ("sn12 DL: %ld\n", item_p->pdcpStatusTransfer_DL.choice.pdcp_sn_12bits->cOUNTValue.pdcp_SN12);

       snstatustran->drb_list.drb[i].pdcp_status_dl.pdcp_sn_12bit.count_value.hfn_pdcp_sn12 = item_p->pdcpStatusTransfer_DL.choice.pdcp_sn_12bits->cOUNTValue.hfn_PDCP_SN12;
       printf ("hfn12 DL: %ld\n", item_p->pdcpStatusTransfer_DL.choice.pdcp_sn_12bits->cOUNTValue.hfn_PDCP_SN12);
       */

       /* 12 bit */

       /* 18bit SN Status Transfer */

       snstatustran->drb_list.drb[i].pdcp_status_ul.pdcp_sn_18bit.count_value.pdcp_sn18 = item_p->pdcpStatusTransfer_UL.choice.pdcp_sn_18bits->cOUNTValue.pdcp_SN18;
       printf ("sn18 UL: %ld\n", item_p->pdcpStatusTransfer_UL.choice.pdcp_sn_18bits->cOUNTValue.pdcp_SN18);

       snstatustran->drb_list.drb[i].pdcp_status_ul.pdcp_sn_18bit.count_value.hfn_pdcp_sn18 = item_p->pdcpStatusTransfer_UL.choice.pdcp_sn_18bits->cOUNTValue.hfn_PDCP_SN18;
       printf ("hfn18 UL: %ld\n", item_p->pdcpStatusTransfer_UL.choice.pdcp_sn_18bits->cOUNTValue.hfn_PDCP_SN18);

       snstatustran->drb_list.drb[i].pdcp_status_dl.pdcp_sn_18bit.count_value.pdcp_sn18 = item_p->pdcpStatusTransfer_DL.choice.pdcp_sn_18bits->cOUNTValue.pdcp_SN18;
       printf ("sn18 DL: %ld\n", item_p->pdcpStatusTransfer_DL.choice.pdcp_sn_18bits->cOUNTValue.pdcp_SN18);

       snstatustran->drb_list.drb[i].pdcp_status_dl.pdcp_sn_18bit.count_value.hfn_pdcp_sn18 = item_p->pdcpStatusTransfer_DL.choice.pdcp_sn_18bits->cOUNTValue.hfn_PDCP_SN18;
       printf ("hfn18 DL: %ld\n", item_p->pdcpStatusTransfer_DL.choice.pdcp_sn_18bits->cOUNTValue.hfn_PDCP_SN18);
