...
asn_dec_rval_t
SET_OF_decode_uper_e2ap_v1_01(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    do {
        int i;
        if(nelems < 0) {
            nelems = uper_get_length_e2ap_v1_01(pd, -1, 0, &repeat);
...
asn_enc_rval_t
SET_OF_encode_uper_e2ap_v1_01(const asn_TYPE_descriptor_t *td,
    ...
    if(ct && ct->effective_bits >= 0) {
         ...
         */
        if (uper_put_length_e2ap_v1_01(po, 0, 0)) {
            ASN__ENCODE_FAILED;
        }
        ASN__ENCODED_OK(er);
    }
     ...
     */
    encoded_els = SET_OF__encode_sorted_e2ap_v1_01(elm, list, SOES_CUPER);
    ...
    for(encoded_edx = 0; (ssize_t)encoded_edx < list->count;) {
        ...
        if(ct && ct->effective_bits >= 0) {
            ...
            may_encode =
                uper_put_length_e2ap_v1_01(po, list->count - encoded_edx, &need_eom);
