...
OBJECT_IDENTIFIER__biased_random_arc(asn_oid_arc_t upper_bound) {
    ...
    switch(asn_random_between_e2ap_v1_01(0, 2)) {
    case 0:
        idx = asn_random_between_e2ap_v1_01(0, sizeof(values) / sizeof(values[0]) - 1);
        if(values[idx] < upper_bound) {
            return values[idx];
        }
        /* Fall through */
    case 1:
        return asn_random_between_e2ap_v1_01(0, upper_bound);
    case 2:
    default:
        return upper_bound;
    }
...
OBJECT_IDENTIFIER_random_fill_e2ap_v1_01(const asn_TYPE_descriptor_t *td, void **sptr,
    ...
    size_t arcs_len = asn_random_between_e2ap_v1_01(2, 5);
    size_t i;
    ...
    arcs[0] = asn_random_between_e2ap_v1_01(0, 2);
    ...
    if(OBJECT_IDENTIFIER_set_arcs_e2ap_v1_01(st, arcs, arcs_len)) {
        if(st != *sptr) {
