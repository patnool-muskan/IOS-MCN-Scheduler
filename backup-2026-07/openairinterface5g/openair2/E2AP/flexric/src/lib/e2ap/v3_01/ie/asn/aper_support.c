...
int32_t
aper_get_align_e2ap_v3_01(asn_per_data_t *pd) {
...
ssize_t
aper_get_length_e2ap_v3_01(asn_per_data_t *pd, ssize_t lb, ssize_t ub,
	...
	*repeat = 0;
	...
	if (constrained && ub < 65536) {
		int range = ub - lb + 1;
		return aper_get_nsnnwn_e2ap_v3_01(pd, range);
	}
...
ssize_t
aper_get_nslength_e2ap_v3_01(asn_per_data_t *pd) {
	ssize_t length;
	...
	if(per_get_few_bits(pd, 1) == 0) {
		...
		int repeat;
		length = aper_get_length_e2ap_v3_01(pd, -1, -1, -1, &repeat);
...
ssize_t
aper_get_nsnnwn_e2ap_v3_01(asn_per_data_t *pd, int range) {
	ssize_t value;
	int bytes = 0;
...
int aper_put_align_e2ap_v3_01(asn_per_outp_t *po) {
...
ssize_t
aper_put_length_e2ap_v3_01(asn_per_outp_t *po, ssize_t lb, ssize_t ub, size_t n, int *need_eom) {
	int constrained = (lb >= 0) && (ub >= 0);
	int dummy = 0;
	...
	*need_eom = 0;
	...
	if (constrained && ub < 65536) {
		int range = ub - lb + 1;
		return aper_put_nsnnwn_e2ap_v3_01(po, range, n) ? -1 : (ssize_t)n;
	}
...
int
aper_put_nslength_e2ap_v3_01(asn_per_outp_t *po, size_t length) {
	...
	if(length <= 64) {
		...
		if(aper_put_length_e2ap_v3_01(po, -1, -1, length, NULL) != (ssize_t)length) {
...
int
aper_put_nsnnwn_e2ap_v3_01(asn_per_outp_t *po, int range, int number) {
	int bytes;
	...
	if(aper_put_align_e2ap_v3_01(po) < 0) /* Aligning on octet */
		return -1;
