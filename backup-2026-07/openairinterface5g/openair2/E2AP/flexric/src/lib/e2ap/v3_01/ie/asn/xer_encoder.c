 ...
 */
asn_enc_rval_t
xer_encode_e2ap_v3_01(const asn_TYPE_descriptor_t *td, const void *sptr,
           enum xer_encode_e2ap_v3_01r_flags_e xer_flags, asn_app_consume_bytes_f *cb,
           void *app_key) {
	...
	tmper = td->op->xer_encode_e2ap_v3_01r(td, sptr, 1, xer_flags, cb, app_key);
...
/*
 * This is a helper function for xer_fprint_e2ap_v3_01, which directs all incoming data
...
int
xer_fprint_e2ap_v3_01(FILE *stream, const asn_TYPE_descriptor_t *td, const void *sptr) {
	asn_enc_rval_t er = {0,0,0};
	...
	er = xer_encode_e2ap_v3_01(td, sptr, XER_F_BASIC, xer__print2fp, stream);
	if(er.encoded == -1)
		return -1;
...
xer_equivalent_e2ap_v3_01(const struct asn_TYPE_descriptor_s *td, const void *struct1,
    ...
    e1 = xer_encode_e2ap_v3_01(td, struct1, XER_F_BASIC, xer__buffer_append, &xb1);
    if(e1.encoded == -1) {
        if(opt_debug_stream) {
    ...
    e2 = xer_encode_e2ap_v3_01(td, struct2, XER_F_BASIC, xer__buffer_append, &xb2);
    if(e2.encoded == -1) {
        if(opt_debug_stream) {
            fprintf(stderr, "XER Encoding of %s failed\n", td->name);
        }
    ...
    rval = xer_decode_e2ap_v3_01(NULL, td, (void **)&sptr, xb1.buffer,
               xb1.buffer_size);
    switch(rval.code) {
    case RC_OK:
        break;
    ...
    if(rval.consumed != xb1.buffer_size
       && ((rval.consumed > xb1.buffer_size)
           || xer_whitespace_span_e2ap_v3_01(xb1.buffer + rval.consumed,
    ...
    e2 = xer_encode_e2ap_v3_01(td, sptr, XER_F_BASIC, xer__buffer_append, &xb2);
    if(e2.encoded == -1) {
        if(opt_debug_stream) {
