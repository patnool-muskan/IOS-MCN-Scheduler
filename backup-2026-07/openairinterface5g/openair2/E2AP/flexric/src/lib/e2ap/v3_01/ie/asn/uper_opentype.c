 ...
 */
int
uper_open_type_put_e2ap_v3_01(const asn_TYPE_descriptor_t *td,
    ...
    size = uper_encode_e2ap_v3_01_to_new_buffer(td, constraints, sptr, &buf);
    if(size <= 0) return -1;
    ...
    do {
        int need_eom = 0;
        ssize_t may_save = uper_put_length_e2ap_v3_01(po, size, &need_eom);
...
uper_open_type_get_e2ap_v3_01_simple(const asn_codec_ctx_t *ctx,
	...
	do {
		chunk_bytes = uper_get_length_e2ap_v3_01(pd, -1, 0, &repeat);
		if(chunk_bytes < 0) {
			FREEMEM(buf);
			ASN__DECODE_STARVED;
		}
	...
	rv = td->op->uper_decode_e2ap_v3_01r(ctx, td, constraints, sptr, &spd);
	ASN_DEBUG_INDENT_ADD(-4);
...
uper_open_type_get_e2ap_v3_01_complex(const asn_codec_ctx_t *ctx,
                           const asn_TYPE_descriptor_t *td,
	...
	ASN_DEBUG("Getting open type %s from %s", td->name,
		asn_bit_data_string_e2ap_v3_01(pd));
	arg.oldpd = *pd;
	arg.unclaimed = 0;
	arg.ot_moved = 0;
	arg.repeat = 1;
	...
	ASN_DEBUG_INDENT_ADD(+4);
	rv = td->op->uper_decode_e2ap_v3_01r(ctx, td, constraints, sptr, pd);
	ASN_DEBUG_INDENT_ADD(-4);
...
#define	UPDRESTOREPD	do {						\
	...
	ASN_DEBUG("OpenType %s pd%s old%s unclaimed=%d, repeat=%d", td->name,
		asn_bit_data_string_e2ap_v3_01(pd),
		asn_bit_data_string_e2ap_v3_01(&arg.oldpd),
	...
	if(pd->nboff != pd->nbits) {
		ASN_DEBUG("Open type %s overhead pd%s old%s", td->name,
			asn_bit_data_string_e2ap_v3_01(pd), asn_bit_data_string_e2ap_v3_01(&arg.oldpd));
		if(1) {
			UPDRESTOREPD;
			ASN__DECODE_FAILED;
		} else {
...
asn_dec_rval_t
uper_open_type_get_e2ap_v3_01(const asn_codec_ctx_t *ctx, const asn_TYPE_descriptor_t *td,
                   const asn_per_constraints_t *constraints, void **sptr,
                   asn_per_data_t *pd) {
    return uper_open_type_get_e2ap_v3_01_simple(ctx, td, constraints, sptr, pd);
}

int
uper_open_type_skip_e2ap_v3_01(const asn_codec_ctx_t *ctx, asn_per_data_t *pd) {
	...
	s_td.op = &s_op;
    s_op.uper_decode_e2ap_v3_01r = uper_sot_suck_e2ap_v3_01;

	rv = uper_open_type_get_e2ap_v3_01(ctx, &s_td, 0, 0, pd);
	if(rv.code != RC_OK)
		return -1;
	else
		return 0;
...
uper_ugot_refill(asn_per_data_t *pd) {
	...
	next_chunk_bytes = uper_get_length_e2ap_v3_01(oldpd, -1, 0, &arg->repeat);
	...
	ASN_DEBUG("Refilled pd%s old%s",
		asn_bit_data_string_e2ap_v3_01(pd), asn_bit_data_string_e2ap_v3_01(oldpd));
	return 0;
}
