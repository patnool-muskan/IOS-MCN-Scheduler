 ...
 */
ssize_t
uper_get_length_e2ap_v3_01(asn_per_data_t *pd, int ebits, size_t lower_bound,
                int *repeat) {
    ssize_t value;
 ...
 */
ssize_t
uper_get_nslength_e2ap_v3_01(asn_per_data_t *pd) {
	ssize_t length;
	...
	if(per_get_few_bits(pd, 1) == 0) {
		...
		int repeat;
		length = uper_get_length_e2ap_v3_01(pd, -1, 0, &repeat);
 ...
 */
ssize_t
uper_get_nsnnwn_e2ap_v3_01(asn_per_data_t *pd) {
	ssize_t value;
 ...
 */
int
uper_put_nsnnwn_e2ap_v3_01(asn_per_outp_t *po, int n) {
	int bytes;
...
int uper_get_constrained_whole_number_e2ap_v3_01(asn_per_data_t *pd, uintmax_t *out_value, int nbits) {
	...
	if(uper_get_constrained_whole_number_e2ap_v3_01(pd, &lhalf, nbits - 31))
		return -1;
...
/* X.691-2008/11, #11.5.6 -> #11.3 */
int
uper_put_constrained_whole_number_u_e2ap_v3_01(asn_per_outp_t *po, uintmax_t v,
                                    int nbits) {
    if(nbits <= 31) {
        return per_put_few_bits(po, v, nbits);
    } else {
        /* Put higher portion first, followed by lower 31-bit */
        if(uper_put_constrained_whole_number_u_e2ap_v3_01(po, v >> 31, nbits - 31))
            return -1;
 ...
 */
ssize_t
uper_put_length_e2ap_v3_01(asn_per_outp_t *po, size_t length, int *need_eom) {
    int dummy = 0;
 ...
 */
int
uper_put_nslength_e2ap_v3_01(asn_per_outp_t *po, size_t length) {
    if(length <= 64) {
        /* #11.9.3.4 */
        if(length == 0) return -1;
        ...
        int need_eom = 0;
        if(uper_put_length_e2ap_v3_01(po, length, &need_eom) != (ssize_t)length
           || need_eom) {
...
int
per_imax_range_rebase_e2ap_v3_01(intmax_t v, intmax_t lb, intmax_t ub, uintmax_t *output) {
    uintmax_t range;
...
int
per_long_range_rebase_e2ap_v3_01(long v, intmax_t lb, intmax_t ub, unsigned long *output) {
    uintmax_t tmp = *output;
    int rc = per_imax_range_rebase_e2ap_v3_01((intmax_t)v, lb, ub, &tmp);
    *output = tmp;
    return rc;
}

int
per_imax_range_unrebase_e2ap_v3_01(uintmax_t inp, intmax_t lb, intmax_t ub, intmax_t *outp) {
    uintmax_t range;
...
int
per_long_range_unrebase_e2ap_v3_01(unsigned long inp, intmax_t lb, intmax_t ub, long *outp) {
    intmax_t tmp = *outp;
    int rc = per_imax_range_unrebase_e2ap_v3_01((uintmax_t)inp, lb, ub, &tmp);
    *outp = tmp;
    return rc;
}
