 ...
 */
asn_enc_rval_t
der_encode_e2ap_v2_03(const asn_TYPE_descriptor_t *type_descriptor, const void *struct_ptr,
	 ...
	 */
    return type_descriptor->op->der_encode_e2ap_v2_03r(
        type_descriptor, struct_ptr, /* Pointer to the destination structure */
        0, 0, consume_bytes, app_key);
}

/*
 * Argument type and callback necessary for der_encode_e2ap_v2_03_to_buffer().
 */
...
/*
 * A variant of the der_encode_e2ap_v2_03() which encodes the data into the provided buffer
 */
asn_enc_rval_t
der_encode_e2ap_v2_03_to_buffer(const asn_TYPE_descriptor_t *type_descriptor,
	...
	ec = type_descriptor->op->der_encode_e2ap_v2_03r(type_descriptor,
 ...
 */
ssize_t
der_write_tags_e2ap_v2_03(const asn_TYPE_descriptor_t *sd, size_t struct_length,
...
#define ASN1_DER_MAX_TAGS_COUNT 4
    ...
    ASN_DEBUG("Writing tags (%s, tm=%d, tc=%d, tag=%s, mtc=%d)",
		...
		ber_tlv_tag_string_e2ap_v2_03(tag),
		tag_mode
			?(sd->tags_count+1
	...
	if(tag_mode) {
		/*
		 * Instead of doing shaman dance like we do in ber_check_tags_e2ap_v2_03(),
...
der_write_TL(ber_tlv_tag_t tag, ber_tlv_len_t len,
	...
	tmp = ber_tlv_tag_serialize_e2ap_v2_03(tag, buf, buf_size);
	if(tmp == -1 || tmp > (ssize_t)sizeof(buf)) return -1;
	size += tmp;

	/* Serialize length (L from TLV) into possibly zero-length buffer */
	tmp = der_tlv_length_serialize_e2ap_v2_03(len, buf+size, buf_size?buf_size-size:0);
	if(tmp == -1) return -1;
	size += tmp;
