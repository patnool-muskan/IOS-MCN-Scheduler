...
asn_dec_rval_t
OCTET_STRING_decode_uper_e2ap_v3_01(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    do {
        ...
        ret = OCTET_STRING_per_get_characters_e2ap_v3_01(pd, &st->buf[st->size], raw_len,
...
asn_enc_rval_t
OCTET_STRING_encode_uper_e2ap_v3_01(const asn_TYPE_descriptor_t *td,
    ...
    if(csiz->effective_bits >= 0 && !inext) {
        ...
        ret = OCTET_STRING_per_put_characters_e2ap_v3_01(po, st->buf, size_in_units, bpc,
    ...
    do {
        int need_eom = 0;
        ssize_t may_save = uper_put_length_e2ap_v3_01(po, size_in_units, &need_eom);
        if(may_save < 0) ASN__ENCODE_FAILED;

        ASN_DEBUG("Encoding %" ASN_PRI_SSIZE " of %" ASN_PRI_SIZE "%s", may_save, size_in_units,
                  need_eom ? ",+EOM" : "");

        ret = OCTET_STRING_per_put_characters_e2ap_v3_01(po, buf, may_save, bpc, unit_bits,
                                              cval->lower_bound,
                                              cval->upper_bound, pc);
        if(ret) ASN__ENCODE_FAILED;
