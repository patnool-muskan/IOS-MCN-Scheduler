...
/* Xn code for E1AP procedures */

/*
typedef enum bearer_context_pdcp_sn_status_req_e {
requested
} bearer_context_pdcp_sn_status_req_t;
*/

typedef struct PDCP_count_s {
long pdcp_sn;
uint32_t hfn;
}PDCP_count_t;

typedef struct DRBBStatusTransfer_s {
   PDCP_count_t countValue;
}  DRBBStatusTransfer_t;

typedef struct bearer_context_pdcp_sn_status_info_s {
   DRBBStatusTransfer_t pdcp_status_transfer_ul;
   PDCP_count_t pdcp_status_transfer_dl;
}bearer_context_pdcp_sn_status_info_t;
...
typedef struct DRB_nGRAN_to_modify_s {
  ...
  e1_pdcp_status_info_t *pdcp_status; /* N2 handover code for E1AP SN Status information procedures */
#endif

  bearer_context_pdcp_sn_status_info_t pdcp_sn_status_info; /* Xn code for E1AP SN Status information procedures */
...
typedef struct pdu_session_to_setup_s {
  ...
  int *inactivityTimer;
  // Existing Allocated NG DL UP Transport Layer Information (O)
  UP_TL_information_t *existing_DL_UP_TL_info;
  // Network Instance (O)
  uint8_t *networkInstance;
  // DRB To Setup List (M)
  int numDRB2Setup;
...
typedef struct e1ap_bearer_modif_resp_s {
  ...
  int sn_status_req; /* Xn code for E1AP SN status transfer procedures */
  int sn_status_info; /* Xn code for E1AP SN Status transfer procedures */
