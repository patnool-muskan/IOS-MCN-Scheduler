                                    ...
                                    char *_buffer, int size)
{
  ...
  if (entity->has_integrity) {
    ...
    if (memcmp(integrity, buffer + size - integrity_size, PDCP_INTEGRITY_SIZE) != 0) {
      LOG_E(PDCP, "discard NR PDU, integrity failed %d\n", entity->type);
      entity->stats.rxpdu_dd_pkts++;
      entity->stats.rxpdu_dd_bytes += size;
                                      ...
                                      int pdu_max_size)
{
  ...
  LOG_D(XNAP, "nr_pdcp_entity_process_sdu called sdu_id %d size %d has_integrity %d\n",
                  sdu_id, size, entity->has_integrity);
  ...
  if (entity->do_drop && entity->rlc_tx_freesize > 0 ) {
    LOG_D(PDCP, " sdu - rlc tx freesize %d, pdu size = %d \n",entity->rlc_tx_freesize,pdu_max_size);
    if (entity->type != NR_PDCP_SRB)
      if (pdu_max_size > entity->rlc_tx_freesize)
        return -1;
  ...
  LOG_D(XNAP, "In nr_pdcp_entity_process_sdu: entity->type %d entity->sn_size %d  header_size %d has_integrity %d integrity_size %d size %d\n", entity->type, entity->sn_size, header_size, entity->has_integrity, integrity_size, size);
  ...
  if (entity->has_integrity) {
    ...
    LOG_D(XNAP, "size %d integrity_size %d entity_rb_id %d count %d \n",
                                 size,
                                 integrity_size,
                                 entity->rb_id,
                                 count); /* Testing */
    ...
    int dl_srb_count_target)
{
  ...
  ret->is_gnb = is_gnb;
  if (dl_srb_count_target){
     ret->tx_next = dl_srb_count_target;
  }
