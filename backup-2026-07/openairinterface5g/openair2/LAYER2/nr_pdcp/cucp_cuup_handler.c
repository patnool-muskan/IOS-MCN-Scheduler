...
static void fill_gtpu_n3_req(gtpv1u_gnb_create_tunnel_req_t *req_n3, const pdu_session_to_setup_t *req_pdu, int tunnel_id)
{
  ...
  memcpy(&req_n3->dst_addr.buffer, &req_pdu->UP_TL_information.tlAddress, sizeof(uint8_t) * 4);
  req_n3->dst_addr.length = 32;
  req_n3->outgoing_teid = req_pdu->UP_TL_information.teId;
}
...
void e1_bearer_context_setup(const e1ap_bearer_setup_req_t *req)
{
  ...
  LOG_I(E1AP, "E1 bearer context setup  \n");
  ...
  for (int i = 0; i < resp.numPDUSessions; ++i) {
    ...
    for (int d = 0; d < resp_pdu->numDRBSetup; d++) {
      ...
      if (f1inst >= 0) { /* we have F1(-U) */
        ...
        memcpy(&resp_drb->UpParamList[d].tl_info.tlAddress, &resp_f1.gnb_addr.buffer, 4);
        resp_drb->UpParamList[d].tl_info.teId = resp_f1.gnb_NGu_teid;
        LOG_D(E1AP, "Created F1-U tunnel for DRB %ld (Teid %d tlAddress %x)\n",
              req_drb->id,
              resp_drb->UpParamList[d].tl_info.teId,
              resp_drb->UpParamList[d].tl_info.tlAddress);
      }
    }
    /* This is to be detleted - kept for reference
...
*/
    ...
    */
    ...
    resp_pdu->tl_info.teId = resp_n3.gnb_NGu_teid;
    memcpy(&resp_pdu->tl_info.tlAddress, &resp_n3.gnb_addr.buffer, 4);
  }
...
#else
      /* Xn code for E1AP SN status tranfer information procedures */
      modif.sn_status_req = 0;
      if (to_modif->pdcp_sn_status_requested) {
       modif.sn_status_req = 1;

        nr_pdcp_ue_t *ue; 
        nr_pdcp_ue_manager_t *nr_pdcp_ue_manager;

        nr_pdcp_ue_manager = nr_pdcp_sdap_get_ue_manager();
        ue = nr_pdcp_manager_get_ue(nr_pdcp_ue_manager, req->gNB_cu_up_ue_id);

        int              rx_deliv_sn;
        int              tx_deliv_sn;
        uint32_t         rx_deliv_hfn;
        uint32_t         tx_deliv_hfn;
        rx_deliv_sn  = ue->drb[0]->rx_next & ue->drb[0]->sn_max;
        rx_deliv_hfn = ue->drb[0]->rx_next >> ue->drb[0]->sn_size;
        tx_deliv_sn  = ue->drb[0]->tx_next & ue->drb[0]->sn_max;
        tx_deliv_hfn = ue->drb[0]->tx_next >> ue->drb[0]->sn_size;

        modified->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn = tx_deliv_sn;
        modified->pdcp_sn_status_info.pdcp_status_transfer_dl.hfn = tx_deliv_hfn;
        modified->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.pdcp_sn = rx_deliv_sn;
        modified->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.hfn = rx_deliv_hfn;
        printf("[SN Status Request] rx_deliv_sn is %d rx_deliv_hfn is %d tx_deliv_sn is %d tx_deliv_hfn is %d \n", rx_deliv_sn, rx_deliv_hfn, tx_deliv_sn, tx_deliv_hfn);
      }

      /* Xn code for E1AP SN status transfer information procedures */
      if (to_modif->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn) {
        nr_pdcp_ue_t *ue; 
        nr_pdcp_ue_manager_t *nr_pdcp_ue_manager;
        ...
        uint32_t         rx_deliv_hfn;
        uint32_t         tx_deliv_hfn;
        tx_deliv_sn  = to_modif->pdcp_sn_status_info.pdcp_status_transfer_dl.pdcp_sn;
        tx_deliv_hfn = to_modif->pdcp_sn_status_info.pdcp_status_transfer_dl.hfn;
        rx_deliv_sn  = to_modif->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.pdcp_sn;
        rx_deliv_hfn = to_modif->pdcp_sn_status_info.pdcp_status_transfer_ul.countValue.hfn;

        ue->drb[0]->tx_next = (tx_deliv_hfn << ue->drb[0]->sn_size) | tx_deliv_sn;
        ue->drb[0]->rx_next = (rx_deliv_hfn << ue->drb[0]->sn_size) | rx_deliv_sn;
        printf("[SN Status info] tx_next is %d rx_next is %d \n", ue->drb[0]->tx_next, ue->drb[0]->rx_next);
      }
#endif
