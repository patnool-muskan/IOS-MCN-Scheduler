...
#define NUM_PDCCH_AGGREGATION_LEVELS 5
                      ...
                      frame_t frame)
{
  ...
  {
    LOG_D(NR_MAC, "diff is less than update frames : frame %4d MCS %d BLER avg %.6f)\n",
        frame, old_mcs,bler_stats->bler);
  ...
  LOG_D(NR_MAC, "frame %4d MCS %d -> %d (num_dl_sched %d, num_dl_retx %d, BLER wnd %.3f avg %.6f)\n",
                  ...
                  NR_UE_UL_BWP_t *ul_bwp)
{
 ...
 if(pusch_pdu->rb_start >= 65530)
        LOG_I(NR_MAC, "config_uldci - before rballoc");
  ...
  case NR_DL_DCI_FORMAT_1_0:
    switch (rnti_type) {
      ...
      case TYPE_P_RNTI_:
        ...
        int pos = 0;  // IMPORTANT: must start at 0

       //---------------------------------------------------------
       // 1. Short Message Indicator (2 bits) – MSB to LSB
       //---------------------------------------------------------
       uint8_t smi = dci_pdu_rel15->short_messages_indicator;
       for (int i = 1; i >= 0; i--) {
           *dci_pdu |= (((uint64_t)((smi >> i) & 1)) << (dci_size - 1 - pos));
         pos++;
       }

       //---------------------------------------------------------
       // 2. Short Message (8 bits) – MSB to LSB
       //---------------------------------------------------------
       uint8_t sm = dci_pdu_rel15->short_messages;
       for (int i = 7; i >= 0; i--) {
           *dci_pdu |= (((uint64_t)((sm >> i) & 1)) << (dci_size - 1 - pos));
         pos++;
       }

      //---------------------------------------------------------
      // SMI == 10 (short messages only)
      // -> Skip scheduling fields and write reserved (fsize + 12 bits)
      //---------------------------------------------------------
      if (smi == 0b10) {
        int reserved_bits = fsize + 12;
        pos += reserved_bits;
      } else {
      //---------------------------------------------------------
      // 3. Frequency Domain Assignment (fsize bits)
      //---------------------------------------------------------
      uint32_t f_alloc = dci_pdu_rel15->frequency_domain_assignment.val;

        for (int i = fsize - 1; i >= 0; i--) {
             *dci_pdu |= ((((uint64_t)(f_alloc >> i)) & 1)
                        << (dci_size - 1 - pos));
          pos++;
        }

      //---------------------------------------------------------
      // 4. Time Domain Assignment (4 bits)
      //---------------------------------------------------------
      uint8_t tda = dci_pdu_rel15->time_domain_assignment.val;

        for (int i = 3; i >= 0; i--) {
             *dci_pdu |= ((((uint64_t)((tda >> i) & 1))) << (dci_size - 1 - pos));
          pos++;
        }

      //---------------------------------------------------------
      // 5. VRB → PRB Mapping (1 bit)
      //---------------------------------------------------------
      uint8_t vrb = dci_pdu_rel15->vrb_to_prb_mapping.val & 1;
      *dci_pdu |= ((uint64_t)vrb << (dci_size - 1 - pos));
      pos++;
      ...
      // 7. TB Scaling (2 bits)
      //---------------------------------------------------------
      uint8_t tbs = dci_pdu_rel15->tb_scaling;

      for (int i = 1; i >= 0; i--) {
           *dci_pdu |= ((((uint64_t)(tbs >> i)) & 1)
                       << (dci_size - 1 - pos));
        pos++;
      }
    }
     ...
     //---------------------------------------------------------
     // 8. Reserved bits (6 bits = 0)
     //---------------------------------------------------------
     pos += 6;  // Zero bits → just advance pos
     break;
#else
      case TYPE_P_RNTI_:
...
void delete_nr_ue_data(NR_UE_info_t *UE, uid_allocator_t *uia)
{
  ...
  // Audit
  /*
  TBD Comment- need to move this to audit process
  for (int i = 0; i < NR_NB_RA_PROC_MAX; i++) {
    NR_RA_t *ra = &ccPtr->ra[i];
    if (ra->rnti == UE->rnti) nr_clear_ra_proc(ra);
  } */
...
/* TODO: it seems that if we add this, the gNB might trigger RLF before the UE,
 * which is a problem because then the UE cannot trigger RLF (and follow up
 * with reestab etc). what to do? */
...
bool prepare_initial_ul_rrc_message(gNB_MAC_INST *mac, NR_UE_info_t *UE)
{
  ...
  if (dl->slices) {
    ...
    for (int l = 0; l < seq_arr_size(&sched_ctrl->lc_config); ++l) {
      nr_lc_config_t *c = seq_arr_at(&sched_ctrl->lc_config, l);
      if ( c->lcid == lcid) {
        c->nssai = *default_nssai;
        break;
      }
    }
...
void nr_mac_trigger_ul_failure(NR_UE_sched_ctrl_t *sched_ctrl, NR_SubcarrierSpacing_t subcarrier_spacing)
{
  ...
  sched_ctrl->ul_failure_timer = 1000 << subcarrier_spacing;
// Kept it lesser
...
static void determine_aggregation_level_search_order(int agg_level_search_order[NUM_PDCCH_AGG_LEVELS], float pdcch_cl_adjust)
{
  ...
  if (desired_agg_level_index < 0) desired_agg_level_index = 0;
  if (desired_agg_level_index > NUM_PDCCH_AGG_LEVELS) 
	  desired_agg_level_index = NUM_PDCCH_AGG_LEVELS;
  if (desired_agg_level_index !=0) LOG_D(NR_MAC, " %d  desired level index \n", desired_agg_level_index);
