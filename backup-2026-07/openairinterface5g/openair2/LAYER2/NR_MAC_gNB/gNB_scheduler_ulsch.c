                              ...
                              const int8_t harq_pid)
{
  ...
  while (pdu_len > 0) {
    ...
    if (mac_subheader_len + mac_len > pdu_len) {
      LOG_E(NR_MAC,
            "Invalid PDU with LCID 0x%X in %d.%d for RNTI %04X! mac_subheader_len: %d, mac_len: %d, remaining pdu_len: %d\n",
            lcid,
            frameP,
            slot,
            UE->rnti,
    ...
    nr_clear_ra_proc(ra, gNB_mac);
     // Audit
        ...
        for (int i = 0; i < gNB->max_nb_pucch; i++) {
          ...
          if ((pucch->frame != 0) || (pucch->slot !=0))
	        if (((pucch->frame == ra->Msg2_frame) && ( pucch->slot == ra->Msg2_slot ))  
                || ((pucch->frame == ra->Msg3_frame) && ( pucch->slot == ra->Msg3_slot )) 
                || ((pucch->frame == ra->preamble_frame) && ( pucch->slot == ra->preamble_slot ))) {
       LOG_I(NR_MAC, "%s():  Audit - PUCCH Reset %d - %d \n", __func__, pucch->frame, pucch->slot);
                    pucch->active = 0;
                }
        } 
#endif
                                ...
                                nfapi_nr_srs_indication_pdu_t *srs_ind)
{
  ...
  if (srs_ind->timing_advance_offset == 0xFFFF) {
    LOG_D(NR_MAC, "Invalid timing advance offset for RNTI %04x\n", srs_ind->rnti);
    NR_SCHED_UNLOCK(&nrmac->sched_lock);
    return;
  }
                                 ...
                                 uint8_t *mcs)
{
  ...
  int tx_power = compute_ph_factor(mu,
                                   ...
                                   *Rb,
                                   ...
                                   deltaMCS,
                                   true);
  LOG_D(NR_MAC, "Checking %d RBs, MCS %d, ph_limit %d, tx_power %d, min_rb %d\n",*Rb,*mcs,ph_limit,tx_power,minRb);
  if ( *Rb > minRb){
    int hi = *Rb;
    int lo = minRb;
				       ...
				       const int tda)
{
  ...
  /* Find a free CCE */
  LOG_D(NR_MAC, "[UE %04x] %lf cl adjust retransmission UL DCI\n", UE->rnti, sched_ctrl->pdcch_cl_adjust);
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (bo->harq_round_max == 1) {
      ...
      sched_pusch->mcs = get_mcs_from_bler(bo, stats, &sched_ctrl->ul_bler_stats, max_mcs_table, frame);
    ...
    if (B == 0 && do_sched) {
      ...
      /* Find a free CCE */
      LOG_D(NR_MAC, "[UE %04x] %lf cl adjust inactivity UL DCI\n", UE->rnti, sched_ctrl->pdcch_cl_adjust);
      int CCEIndex = get_cce_index(nrmac,
                                   CC_id, slot, UE->rnti,
                                   &sched_ctrl->aggregation_level,
                                   dci_beam.idx,
  ...
  while (iterator->UE != NULL) {
    ...
    uint16_t rbSize = 0;
    uint32_t TBS = 0;
    bool res = nr_find_nb_rb(sched_pusch->Qm,
                  sched_pusch->R,
                  current_BWP->transform_precoding,
                  sched_pusch->nrOfLayers,
                  sched_pusch->tda_info.nrOfSymbols,
    ...
    if (!res) {
      LOG_D(NR_MAC, "[UE %04x] ULSCH rbsize %d TBS %d layers %d symbols %d dmrs %d total bytes %d minrb %d maxrb %d \n",
            iterator->UE->rnti, rbSize,TBS, sched_pusch->nrOfLayers, sched_pusch->tda_info.nrOfSymbols,sched_pusch->dmrs_info.N_PRB_DMRS * sched_pusch->dmrs_info.num_dmrs_symb,B,
                  min_rb, max_rbSize);
    }
...
static bool nr_ulsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  NR_PUSCH_TimeDomainResourceAllocationList_t *tdaList = get_ul_tdalist(current_BWP,
                                                                        ...
                                                                        sched_ctrl->search_space->searchSpaceType->present,
                                                                        TYPE_C_RNTI_);
  const int K2 = get_K2(tdaList, temp_tda, mu, scc);
  ...
  UE_iterator(nr_mac->UE_info.list, UE2) {
    ...
    NR_UE_UL_BWP_t *current_BWP = &UE2->current_UL_BWP;
    int mu = current_BWP->scs;
    NR_PUSCH_TimeDomainResourceAllocationList_t *tdaList = get_ul_tdalist(current_BWP,
                                                                          sched_ctrl->coreset->controlResourceSetId,
                                                                          sched_ctrl->search_space->searchSpaceType->present,
    ...
    const int tda = get_ul_tda(nr_mac, sched_ctrl->sched_pusch.frame, sched_ctrl->sched_pusch.slot);
  ...
  int len[num_beams];
  memset(len, bw, num_beams * sizeof(int));
...
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, sub_frame_t slot, nfapi_nr_ul_dci_request_t *ul_dci_req)
{
  ...
  UE_iterator( UE_info->list, UE) {
    ...
    /** If the RNTI is stored in the RA processes, ULSCH scheduling
     * starts after Msg4 generation, or Msg2 in case of handover. Therefore RA
     * state must be consistent and can't be IDLE. If not, there is possibly an
     * on-going handover (the UE context was created but RA was not completed yet).
     * After RA completion, the RA process is cleared, so this check is skipped. */
    bool ho_in_progress = false;
    for (int i = 0; i < NR_NB_RA_PROC_MAX; ++i) {
      NR_RA_t *ra = &nr_mac->common_channels[0].ra[i];
      if (ra->ra_type == RA_4_STEP && ra->rnti == UE->rnti && ra->ra_state == nrRA_gNB_IDLE) {
        LOG_D(NR_PHY, "UE %04x has on-going RA process with incosistent RA state IDLE (possible handover): skip ULSCH scheduling \n", UE->rnti);
        ho_in_progress = true;
        break;
      }
    }
