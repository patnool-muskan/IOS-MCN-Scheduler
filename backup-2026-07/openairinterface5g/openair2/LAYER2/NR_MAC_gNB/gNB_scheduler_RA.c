...
/* TPH Patch 22May2025 - Start*/
//comments
/* 
Issue Description:
An assertion failure occurred in the MAC scheduler during a Msg3 retransmission failure scenario, where the UE transmitted Msg3 at a different frame/slot than what was scheduled and communicated in Msg2 by the MAC.

Resolution:
We resolved the issue by:
• Replacing the assertion with a conditional check for frame/slot mismatch.
• Freeing all resources allocated to the UE during the Random Access (RA) procedure when such a mismatch is detected.
• This is done using nr_clear_ra_proc(ra) function

Handled the issue by freeing all resources allocated to the UE during Msg2. This ensures the MAC can release all allocated resources gracefully to avoid any further resource hangs issues and thus crash.
*/	  
	  
	  
//TPH_DEBUG
    if(future_ul_tti_req->SFN != sched_frame || future_ul_tti_req->Slot != sched_slot)
    {
     
      /* This log is currently added to verifiy the fix. This log shall be move to LOG_I for release */
      LOG_E(NR_MAC,"future UL_tti_req's frame.slot %d.%d does not match PUSCH %d.%d\n",
...
static bool nr_get_Msg3alloc(gNB_MAC_INST *mac,
                             int CC_id,
                             NR_ServingCellConfigCommon_t *scc,
 ...
 */
   ...
   if(future_ul_tti_req->SFN != ra->Msg3_frame || future_ul_tti_req->Slot != ra->Msg3_slot)
   {
       LOG_E(NR_MAC, "future UL_tti_req's frame.slot %d.%d does not match PUSCH %d.%d\n",
               future_ul_tti_req->SFN,
               future_ul_tti_req->Slot,
                             ...
                             nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  bool msg3_ret = nr_get_Msg3alloc(nr_mac, CC_id, scc, slotP, frameP, ra);
  if (!msg3_ret) {
    reset_beam_status(&nr_mac->beam_info, ra->Msg3_frame, ra->Msg3_slot, ra->beam_id, n_slots_frame, ra->Msg3_beam.new_beam);
    reset_beam_status(&nr_mac->beam_info, frameP, slotP, ra->beam_id, n_slots_frame, beam.new_beam);
    return;
