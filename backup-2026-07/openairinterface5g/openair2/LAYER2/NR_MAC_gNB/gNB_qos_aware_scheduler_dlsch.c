...
int lc_comparator(const void *p, const void *q)
{
  ...
  /*
  nr_lc_config_t *lc1 = NULL;
  nr_lc_config_t *lc2 = NULL;

  for (size_t l = 0; l < seq_arr_size(&sched_ctrl1->lc_config); ++l) {
      lc1 = seq_arr_at(&sched_ctrl1->lc_config, l);
      if ( lc1->lcid == sched_lc1->curr_lcid) break;
  }

  for (size_t l = 0; l < seq_arr_size(&sched_ctrl2->lc_config); ++l) {
      lc2 = seq_arr_at(&sched_ctrl2->lc_config, l);
      if ( lc2->lcid == sched_lc2->curr_lcid) break;
  }
*/
                                  ...
                                  void *data)
{
  ...
  UE_iterator (UE_list, UE) {
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      bool r = allocate_dl_retransmission(module_id, frame, slot, &n_rb_sched, UE,0, sched_pdsch->dl_harq_pid);
  ...
  while (remainUEs > 0 && n_rb_sched >= min_rbSize && iterator->UE != NULL) {
    ...
    if (!get_FeedbackDisabled(iterator->UE->sc_info.downlinkHARQ_FeedbackDisabled_r17, sched_pdsch->dl_harq_pid)) {
      ...
      alloc = nr_acknack_scheduling(mac, ul_bwp, sched_ctrl, frame, slot, UE->UE_beam_index, r_pucch, dl_bwp->dci_format);
      if (alloc<0) {
