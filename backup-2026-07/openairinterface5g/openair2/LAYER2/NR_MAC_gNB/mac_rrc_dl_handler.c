                                          ...
                                          NR_CellGroupConfig_t *cellGroupConfig)
{
  ...
  for (int i = 0; i < drbs_len; i++) {
    ...
    while (cellGroupConfig->rlc_BearerToAddModList && idx < cellGroupConfig->rlc_BearerToAddModList->list.count) {
    ...
    if (cellGroupConfig->rlc_BearerToAddModList && idx < cellGroupConfig->rlc_BearerToAddModList->list.count) {
      LOG_I(NR_MAC, "Releasing DRB (LCID %ld) for UE %04x\n", lcid, UE->rnti);
      ...
      *plcid = lcid;
      ...
      LOG_W(NR_MAC, "Requested release of DRB ID %ld (LCID %ld), but not found in AddModList. Skipping.\n",
            drb->rb_id, lcid);
    }
  }

  if (released_count == 0 && cellGroupConfig->rlc_BearerToReleaseList) {
    // Free rlc_BearerToReleaseList since it's unused
    for (int i = 0; i < cellGroupConfig->rlc_BearerToReleaseList->list.count; i++) {
      if (cellGroupConfig->rlc_BearerToReleaseList->list.array[i])
        free(cellGroupConfig->rlc_BearerToReleaseList->list.array[i]);
    }
    free(cellGroupConfig->rlc_BearerToReleaseList->list.array);
    free(cellGroupConfig->rlc_BearerToReleaseList);
...
//    UE->isUEDoingHandover = true;
  ...
  if (req->cu_to_du_rrc_information != NULL) {
    ...
    if (cu2du->measConfig != NULL)
      UE->meas_config = get_nr_meas_config(cu2du->measConfig, cu2du->measConfig_length);
  }
...
{
  LOG_I(MAC, "Received UE Context Modification request for UE %04x\n",req->gNB_DU_ue_id);
  gNB_MAC_INST *mac = RC.nrmac[0];
  ...
  if (req->drbs_to_be_released_length > 0) {
    ...
    LOG_I(MAC, "Requested to release DRBs, checking existence...\n");
  }

  if (req->rrc_container != NULL) {
   logical_chan_id_t id = 1;
   #ifdef UNI_RAN
   LOG_I(MAC, "Received UE Context Modification Request with RRC container for UE %04x (length %d)\n",
     req->gNB_DU_ue_id, req->rrc_container_length);
   #else
   LOG_I(MAC, " Sending ho command - Received UE CMR for UE %04x ,length of rrc container  %d \n",req->gNB_DU_ue_id,req->rrc_container_length);
   #endif
...
#ifdef UNI_RAN
/*Building a MAC PDU for PCCH - PAGING*/
static int nr_build_pcch_mac_pdu(uint8_t *dst, uint8_t *paging_pdu, int paging_len)
{
  int offset = 0;
  /* Copy Paging ASN.1 payload */
  memcpy(&dst[offset], paging_pdu, paging_len);
  offset += paging_len;

  return offset; // total MAC PDU size
}
#endif
...
void dl_rrc_message_transfer(const f1ap_dl_rrc_message_t *dl_rrc)
{
  ...
  if (dl_rrc->srb_id == RRC_DL_PAGING) {
      LOG_D(NR_MAC, "SRB = PCCH\n");
      if (!mac) {
          LOG_E(NR_MAC, "No MAC instance available for PCCH\n");
          return;
      }
      /* Storing paging PDU for later scheduling */
      mac->pcch_pdu_size = nr_build_pcch_mac_pdu(mac->pcch_pdu, dl_rrc->rrc_container, dl_rrc->rrc_container_length);
      mac->pcch_pending = true;

      LOG_D(NR_MAC, "[MAC] Stored Paging PDU (size=%d bytes) for PCCH scheduling\n", dl_rrc->rrc_container_length);
      return;  // <-- IMPORTANT: do not send paging to RLC.
  }
  #endif
...
//  if (UE->expect_reconfiguration && dl_rrc->srb_id == DL_SCH_LCID_DCCH && UE->isUEDoingHandover == false) {
  if (UE && UE->expect_reconfiguration && dl_rrc->srb_id == DL_SCH_LCID_DCCH ) {
  ...
  if (dl_rrc->old_gNB_DU_ue_id != NULL) {
	...
	//TPH_PATCH_START_21_AUG
    if(oldUE==NULL)
    {
        LOG_I(NR_MAC, "CU claims we should know UE %04x, but we don't\n", *dl_rrc->old_gNB_DU_ue_id);
        f1ap_ue_context_release_req_t request = {
       .gNB_CU_ue_id = dl_rrc ->gNB_CU_ue_id,
       .gNB_DU_ue_id = dl_rrc ->gNB_DU_ue_id,
       .cause = F1AP_CAUSE_RADIO_NETWORK,
       .cause_value = F1AP_CauseRadioNetwork_procedure_cancelled,};
       mac->mac_rrc.ue_context_release_request(&request);
       pthread_mutex_unlock(&mac->sched_lock);
...
//TPH_PATCH_END_21_AUG
    ...
    // Safwan: changes still required to see if dl_rcc structure has to be freed properly. 
    if(ra == NULL) {
        LOG_E(NR_MAC, "RA context not found for UE %04x\n", dl_rrc->gNB_DU_ue_id);
        f1ap_ue_context_release_req_t request = {
       .gNB_CU_ue_id = dl_rrc ->gNB_CU_ue_id,
       .gNB_DU_ue_id = dl_rrc ->gNB_DU_ue_id,
       .cause = F1AP_CAUSE_RADIO_NETWORK,
  ...
  if(false ==  is_rlc_ue_available(dl_rrc->gNB_DU_ue_id)) { //Check if RLC Context available or not
    // UE context release Request Triggred, we can change cause and cause value later
    LOG_I(MAC,"RLC Context not available \n");
    f1ap_ue_context_release_req_t request = {
    .gNB_CU_ue_id = dl_rrc ->gNB_CU_ue_id,
    .gNB_DU_ue_id = dl_rrc ->gNB_DU_ue_id,
    .cause = F1AP_CAUSE_RADIO_NETWORK,
