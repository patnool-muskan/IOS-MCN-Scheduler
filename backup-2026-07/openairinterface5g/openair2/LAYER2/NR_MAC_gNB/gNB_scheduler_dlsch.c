...
static dl_bwp_info_t get_bwp_start_size(gNB_MAC_INST *mac, NR_UE_info_t *UE)
{
  ...
  if (bwp_info.bwpStart>bwp_info.bwpSize)
  LOG_W(NR_MAC,"bwpsize %d, bwpstart %d \n",bwp_info.bwpSize,bwp_info.bwpStart);
  return bwp_info;
}
                                       ...
                                       int current_harq_pid)
{
  ...
  * this helps allocate retransmission when TDA changes (e.g. new nrOfSymbols > old nrOfSymbols) */
  NR_tda_info_t temp_tda = get_dl_tda_info(dl_bwp,
                                           ...
                                           false);
  #ifdef UNI_RAN
  if (!temp_tda.valid_tda || temp_tda.nrOfSymbols <= 1 || temp_tda.startSymbolIndex < 0 || temp_tda.k2 < 0)
    return false;
  #else
  ...
  if (reuse_old_tda && layers == retInfo->nrOfLayers) {
    ...
    #ifdef UNI_RAN
    // Try all TDAs until we find one that can retransmit with same or similar TB size
    bool success = false;
    uint32_t new_tbs = 0;
    uint16_t new_rbSize = 0;
    int best_tda_idx = -1;
    NR_tda_info_t best_tda = {0};
    NR_pdsch_dmrs_t best_dmrs = {0};

    // Start TDA loop
    for (int tda_idx = 0; tda_idx < 3; tda_idx++) {
      NR_tda_info_t test_tda = get_dl_tda_info(dl_bwp,
                                               sched_ctrl->search_space->searchSpaceType->present,
                                               tda_idx,
                                               scc->dmrs_TypeA_Position,
                                               1,
                                               ...
                                               coresetid,
                                               false);
      if (!test_tda.valid_tda || test_tda.nrOfSymbols <= 1 || test_tda.startSymbolIndex < 0 || test_tda.k2 < 0)
        continue;

      NR_pdsch_dmrs_t test_dmrs = get_dl_dmrs_params(scc, dl_bwp, &test_tda, layers);

      const uint16_t slbitmap = SL_to_bitmap(test_tda.startSymbolIndex, test_tda.nrOfSymbols);
      ...
      while (rbStart + rbSize <= rbStop && !(rballoc_mask[rbStart + rbSize] & slbitmap))
        rbSize++;
      // Try retransmission with original MCS
      success = nr_find_nb_rb(retInfo->Qm,
                              retInfo->R,
                              1,
                              layers,
                              test_tda.nrOfSymbols,
                              test_dmrs.N_PRB_DMRS * test_dmrs.N_DMRS_SLOT,
                              retInfo->tb_size,
                              1,
                              rbSize,
                              &new_tbs,
      ...
      if (success && new_tbs == retInfo->tb_size) {
        best_tda_idx = tda_idx;
        best_tda = test_tda;
        best_dmrs = test_dmrs;
        break;
      }

      // Try lower Qm/R as fallback (modulation adaptation)
      if (!success || new_tbs != retInfo->tb_size) {
        int fallback_Qm = (retInfo->Qm > 2) ? retInfo->Qm - 2 : retInfo->Qm; // Drop modulation order by 1 level
        int fallback_R  = (retInfo->R > 60) ? retInfo->R - 10 : retInfo->R;  // Slightly reduce code rate

        success = nr_find_nb_rb(fallback_Qm,
                                fallback_R,
                                1,
                                layers,
                                test_tda.nrOfSymbols,
        ...
        if (success && new_tbs <= retInfo->tb_size && new_tbs >= (retInfo->tb_size * 3) / 4) {
          LOG_I(NR_MAC,
                "[UE %04x][%4d.%2d] Harq pid %d fallback: used lower MCS (Qm %d, R %d) for retransmission\n",
                UE->rnti, frame, slot, current_harq_pid, fallback_Qm, fallback_R);
          best_tda_idx = tda_idx;
          best_tda = test_tda;
          best_dmrs = test_dmrs;
          break;
    ...
    if (!success) {
      LOG_I(NR_MAC,
            "[UE %04x][%4d.%2d] Harq pid %d: allocation of DL retransmission failed after trying all TDAs\n",
            UE->rnti, frame, slot, current_harq_pid);
      curInfo->dl_fail_retries++;
      return false;
    }
    #else
    ...
    if (!success || new_tbs != retInfo->tb_size) {
      LOG_I(NR_MAC, "[UE %04x][%4d.%2d] Harq pid %d allocation of DL retransmission failed: new TBS %d of new TDA does not match old TBS %d\n",
            UE->rnti,
            frame,
            slot,
            current_harq_pid,
    ...
    retInfo->time_domain_allocation = best_tda_idx;
    retInfo->tda_info = best_tda;
    retInfo->dmrs_parms = best_dmrs;
  ...
  LOG_D(NR_MAC, "[UE %04x] %lf  cl adjust retransmission DL DCI\n", UE->rnti, sched_ctrl->pdcch_cl_adjust);
  /* Find a free CCE */
  ...
  if (CCEIndex<0) {
    LOG_I(NR_MAC, "[UE %04x][%4d.%2d]  cl_adj %lf could not find free CCE for DL DCI retransmission\n", UE->rnti, frame, slot, sched_ctrl->pdcch_cl_adjust);
    return false;
  }
...
uint32_t pf_tbs[3][29][4]; // pre-computed, approximate TBS values for PF coefficient
...
static void *nr_qos_dl_setup(void)
{
  void *data = malloc(MAX_MOBILES_PER_GNB * sizeof(float));
  AssertFatal(data, "%s(): could not allocate data\n", __func__);
  for (int i = 0; i < MAX_MOBILES_PER_GNB; i++)
...
static void nr_qos_dl_unset(void **data)
{
  DevAssert(data);
  if (*data)
    free(*data);
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      if (sched_ctrl->available_dl_harq.head < 0) {
        LOG_W(NR_MAC, "[UE %04x][%4d.%2d] UE has no free DL HARQ process, skipping\n",
              UE->rnti,
              frame,
              slot);
        continue;
...
//      const uint16_t R = nr_get_code_rate_dl(sched_pdsch->mcs, current_BWP->mcsTableIdx);
      uint32_t tbs = pf_tbs[current_BWP->mcsTableIdx][sched_pdsch->mcs][sched_pdsch->nrOfLayers];
...
//                                    sched_pdsch->nrOfLayers) >> 3;
  ...
  while (iterator->UE != NULL) {
    ...
    * allocation after CCE alloc fail would be more complex) */
    ...
    int total_bytes = 0;
    if (dl->slices) 
      total_bytes = min(sched_ctrl->sliceInfo[sched_ctrl->last_sched_slice].num_total_bytes + oh, 0xfc00);
    else 
      total_bytes = min(sched_ctrl->num_total_bytes + oh, 0xfc00); // to avoid nFAPI packets > 0xffff
    
    bool res = nr_find_nb_rb(sched_pdsch->Qm,
                  sched_pdsch->R,
                  1, // no transform precoding for DL
                  ...
                  &rbSize);
    if (!res) {
      LOG_D(NR_MAC, "[UE %04x]DLSCH rbsize %d TBS %d layers %d symbols %d dmrs %d total bytes %d minrb %d maxrb %d \n",
            iterator->UE->rnti, rbSize,TBS, sched_pdsch->nrOfLayers, tda_info->nrOfSymbols, sched_pdsch->dmrs_parms.N_PRB_DMRS * sched_pdsch->dmrs_parms.N_DMRS_SLOT, total_bytes,
...
nr_dl_sched_algo_t nr_qos_aware_scheduler_dl = {
  .name  = "nr_qos_aware_scheduler_dl",
  .setup = nr_qos_dl_setup,
  .unset = nr_qos_dl_unset,
  .run   = qos_aware_scheduler_dl,
  .data  = NULL
};
...
static void nr_dlsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  int n_rb_sched[num_beams];
  memset(n_rb_sched, bw, num_beams * sizeof(int));
...
nr_pp_impl_param_dl_t nr_init_fr1_dlsch_preprocessor(int CC_id) {
  ...
  for (int mcsTableIdx = 0; mcsTableIdx < 3; ++mcsTableIdx) {
    for (int mcs = 0; mcs < 29; ++mcs) {
      ...
      pf_tbs[mcsTableIdx][mcs][ri] = nr_compute_tbs(Qm,
                                                R,
                                                1, /* rbSize */
                                                10, /* hypothetical number of slots */
                                                0, /* N_PRB_DMRS * N_DMRS_SLOT */
  ...
  if (get_softmodem_params()->use_qos_aware_scheduler == 0) {
    ...
    impl.dl_algo = nr_qos_aware_scheduler_dl;
  }
                         ...
                         nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  UE_iterator(UE_info->list, UE) {
    ...
    if ((sched_pdsch->rbSize <= 0) || (sched_pdsch->rbSize >275)) {
      if (sched_pdsch->dl_fail_retries > 3) 
        if (sched_ctrl->retrans_dl_harq.head >= 0) {
         remove_nr_list(&sched_ctrl->retrans_dl_harq, sched_pdsch->dl_harq_pid);
         finish_nr_dl_harq(sched_ctrl, sched_pdsch->dl_harq_pid);
         LOG_W(NR_MAC, "clearing harq pid %d fail count %d\n",sched_pdsch->dl_harq_pid,sched_pdsch->dl_fail_retries);
        }
      continue;
    }
 ...
 if(pdsch_pdu->rbStart >= 65530)
        LOG_I(NR_MAC, " dlsch - before rballoc");
    ...
    if (harq->round != 0) { /* retransmission */
      ...
      int sdus = 0;
      nr_pp_impl_param_dl_t *dl = &gNB_mac->pre_processor_dl;
      if ( dl->slices &&  sched_ctrl->sliceInfo[sched_ctrl->last_sched_slice].num_total_bytes> 0) {
      ...
      } else if (sched_ctrl->num_total_bytes > 0) {
        /* loop over all activated logical channels */
        for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
          const nr_lc_config_t *c = seq_arr_at(&sched_ctrl->lc_config, i);
          const int lcid = c->lcid;
