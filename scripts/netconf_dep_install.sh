#!/bin/bash
# TODO: Include in scripts to teardown the components installed through this script. - Safwan
...
# Check if libpcre2-dev is installed before attempting installation
$SUDO apt install libpcre2-dev -y

# build and install libssh-dev
cd $INSTALL_PATH
[[ ! -d "libssh" ]] && git clone --single-branch --branch libssh-0.9.2 https://git.libssh.org/projects/libssh.git
...
cd $INSTALL_PATH
[[ ! -d "libyang" ]] && git clone --single-branch --branch v2.1.30 https://github.com/CESNET/libyang.git
...
cd $INSTALL_PATH
[[ ! -d "sysrepo" ]] && git clone --single-branch --branch v2.2.36 https://github.com/sysrepo/sysrepo.git
...
cd $INSTALL_PATH
[[ ! -d "libnetconf2" ]] && git clone --single-branch --branch v2.1.28 https://github.com/CESNET/libnetconf2.git
...
cd $INSTALL_PATH
[[ ! -d "netopeer2" ]] && git clone --single-branch --branch v2.1.49 https://github.com/CESNET/netopeer2.git
...
cd $INSTALL_PATH
[[ ! -d "curl" ]] && git clone --single-branch --branch curl-7_87_0 https://github.com/curl/curl.git
...
cd $INSTALL_PATH
[[ ! -d "cJSON" ]] && git clone --single-branch --branch v1.7.16 https://github.com/DaveGamble/cJSON.git
...
cd $INSTALL_PATH
[[ ! -d "libtelnet" ]] && git clone --single-branch --branch 0.23 https://github.com/seanmiddleditch/libtelnet.git
